...
uses SysUtils, Dialogs, Consts, MMSystem, GraphUtil,
  CommCtrl, ExtActns, ListActns, ActnColorMaps, ImgList, oleacc, ABAccessibility;
...
begin
  case Message.Msg of
    WM_GETOBJECT:
      if (Cardinal(Message.LParam) = OBJID_CLIENT) and (ActionClient <> nil) then
      begin
    ...
    WM_ERASEBKGND:
    begin
      if FDoubleBuffered and (TMessage(Message).wParam <> TMessage(Message).lParam) then
      begin
        Message.Result := 1;
        exit;
      end;
    ...
    VK_DELETE:
      begin
        ...
        if Assigned(Item) then
          ...
          PostMessage(RootMenu.Handle, CM_ITEMSELECTED, 0, LongInt(ParentControl));
        NotifyDesigner(Self);
      end;
  end;
end;
...
begin
  ...
  begin
    SelectedItem := TActionBarClass(Actionbar).FindAccelItem(Message.CharCode);
    if Assigned(SelectedItem) then
    begin
      SelectedItem.Control.Selected := True;
      PostMessage(Menu.Handle, CM_ITEMCLICKED, 0, LongInt(SelectedItem.Control));
      Message.Result := 1;
    end;
  end;
end;
