...
uses
  Consts, Forms, ActiveX, Math, Themes, UxTheme, DwmApi;
...
begin
  ...
  if not (csDestroying in ComponentState) and
    ((Control is TCustomForm) or (Control.FParentWindow <> 0)) then UpdateShowing;
end;
...
begin
  ...
  if AllowWinControls and (FWinControls <> nil) then
    ...
    begin
      ...
      if (LControl = nil) and GetControlAtPos(FWinControls[I]) then
        Break;
    end;
...
begin
  case Message.Msg of
    CM_UNTHEMECONTROL:
      if (csDesigning in ComponentState) and  Assigned(SetWindowTheme) then
      begin
...
begin
  if not (csLoading in ComponentState) and HandleAllocated and not IsZoomed(Handle) then
  begin
...
function TDockTree.InternalHitTest(const MousePos: TPoint;
  out HTFlag: Integer): TDockZone;
var
...
procedure TDockTree.PaintDockFrame(Canvas: TCanvas; Control: TControl;
  ...
  var
    ...
    DrawRect: TRect;
  begin
    DrawRect := Rect(Left, Top, Left+FGrabberSize-2, Top+FGrabberSize-2);
    if ThemeServices.ThemesEnabled then
    begin
...
begin
  if not (csLoading in FDockSite.ComponentState) and
    (FTopZone <> nil) and (FDockSite.VisibleDockClientCount > 0) and
    (FReplacementZone = nil) then
  begin
    R := FDockSite.ClientRect;
    FDockSite.AdjustClientRect(R);
...
begin
  ...
  Stream.Write(FRelativeSizes, SizeOf(FRelativeSizes));
  Stream.Write(FTopZone.FOrientation, SizeOf(FTopZone.FOrientation));
...
begin
  ...
  Dec(ARect.Left, 2 * (ARect.Left - Control.Left));
  Dec(ARect.Top, 2 * (ARect.Top - Control.Top));
  Dec(ARect.Right, 2 * (Control.Width - (ARect.Right - ARect.Left)));
  Temp := 2 * (Control.Height - (ARect.Bottom - ARect.Top));
  if Temp > 0 then
    Dec(ARect.Bottom, Temp);
end;
