...
function BeginBufferedPaint(hdcTarget: HDC; var prcTarget: TRect; 
...
function BufferedPaintMakeOpaque(hBufferedPaint: HPAINTBUFFER; prc: PRect): HResult; inline;
...
var
  ...
  _BeginBufferedPaint: function(hdcTarget: HDC; var prcTarget: TRect; 
...
begin
  ...
  begin
    ...
    begin
      _OpenThemeDataEx := GetProcAddress(ThemeLibrary, 'OpenThemeDataEx'); // Do not localize
      if Assigned(_OpenThemeDataEx) then
        Result := _OpenThemeDataEx(hwnd, pszClassList, dwFlags);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _DrawThemeBackgroundEx := GetProcAddress(ThemeLibrary, 'DrawThemeBackgroundEx'); // Do not localize
      if Assigned(_DrawThemeBackgroundEx) then
        Result := _DrawThemeBackgroundEx(hTheme, hdc, iPartId, iStateId, pRect,
          pOptions);
    end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _SetWindowThemeAttribute := GetProcAddress(ThemeLibrary, 'SetWindowThemeAttribute'); // Do not localize
      if Assigned(_SetWindowThemeAttribute) then
        Result := _SetWindowThemeAttribute(hwnd, eAttribute, pvAttribute, cbAttribute);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _DrawThemeParentBackgroundEx := GetProcAddress(ThemeLibrary, 'DrawThemeParentBackgroundEx'); // Do not localize
      if Assigned(_DrawThemeParentBackgroundEx) then
        Result := _DrawThemeParentBackgroundEx(hwnd, hdc, dwFlags, prc);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetThemeBitmap := GetProcAddress(ThemeLibrary, 'GetThemeBitmap'); // Do not localize
      if Assigned(_GetThemeBitmap) then
        Result := _GetThemeBitmap(hTheme, iPartId, iStateId, iPropId, dwFlags,
          phBitmap);
    end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetThemeStream := GetProcAddress(ThemeLibrary, 'GetThemeStream'); // Do not localize
      if Assigned(_GetThemeStream) then
        Result := _GetThemeStream(hTheme, iPartId, iStateId, iPropId, ppvStream,
          pcbStream, hInst);
    end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintInit := GetProcAddress(ThemeLibrary, 'BufferedPaintInit'); // Do not localize
      if Assigned(_BufferedPaintInit) then
        Result := _BufferedPaintInit;
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintUnInit := GetProcAddress(ThemeLibrary, 'BufferedPaintUnInit'); // Do not localize
      if Assigned(_BufferedPaintUnInit) then
        Result := _BufferedPaintUnInit;
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BeginBufferedPaint := GetProcAddress(ThemeLibrary, 'BeginBufferedPaint'); // Do not localize
      if Assigned(_BeginBufferedPaint) then
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _EndBufferedPaint := GetProcAddress(ThemeLibrary, 'EndBufferedPaint'); // Do not localize
      if Assigned(_EndBufferedPaint) then
        Result := _EndBufferedPaint(hBufferedPaint, fUpdateTarget);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetBufferedPaintTargetRect := GetProcAddress(ThemeLibrary, 'GetBufferedPaintTargetRect'); // Do not localize
      if Assigned(_GetBufferedPaintTargetRect) then
        Result := _GetBufferedPaintTargetRect(hBufferedPaint, prc);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetBufferedPaintTargetDC := GetProcAddress(ThemeLibrary, 'GetBufferedPaintTargetDC'); // Do not localize
      if Assigned(_GetBufferedPaintTargetDC) then
        Result := _GetBufferedPaintTargetDC(hBufferedPaint);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetBufferedPaintDC := GetProcAddress(ThemeLibrary, 'GetBufferedPaintDC'); // Do not localize
      if Assigned(_GetBufferedPaintDC) then
        Result := _GetBufferedPaintDC(hBufferedPaint);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetBufferedPaintBits := GetProcAddress(ThemeLibrary, 'GetBufferedPaintBits'); // Do not localize
      if Assigned(_GetBufferedPaintBits) then
        Result := _GetBufferedPaintBits(hBufferedPaint, ppbBuffer, pcxRow);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintClear := GetProcAddress(ThemeLibrary, 'BufferedPaintClear'); // Do not localize
      if Assigned(_BufferedPaintClear) then
        Result := _BufferedPaintClear(hBufferedPaint, prc);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintSetAlpha := GetProcAddress(ThemeLibrary, 'BufferedPaintSetAlpha'); // Do not localize
      if Assigned(_BufferedPaintSetAlpha) then
        Result := _BufferedPaintSetAlpha(hBufferedPaint, prc, alpha);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintStopAllAnimations := GetProcAddress(ThemeLibrary, 'BufferedPaintStopAllAnimations'); // Do not localize
      if Assigned(_BufferedPaintStopAllAnimations) then
        Result := _BufferedPaintStopAllAnimations(hwnd);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BeginBufferedAnimation := GetProcAddress(ThemeLibrary, 'BeginBufferedAnimation'); // Do not localize
      if Assigned(_BeginBufferedAnimation) then
        Result := _BeginBufferedAnimation(hwnd, hdcTarget, prcTarget, dwFormat, 
          pPaintParams, pAnimationParams, phdcFrom, phdcTo);
    end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _EndBufferedAnimation := GetProcAddress(ThemeLibrary, 'EndBufferedAnimation'); // Do not localize
      if Assigned(_EndBufferedAnimation) then
        Result := _EndBufferedAnimation(hbpAnimation, fUpdateTarget);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _BufferedPaintRenderAnimation := GetProcAddress(ThemeLibrary, 'BufferedPaintRenderAnimation'); // Do not localize
      if Assigned(_BufferedPaintRenderAnimation) then
        Result := _BufferedPaintRenderAnimation(hwnd, hdcTarget);
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _IsCompositionActive := GetProcAddress(ThemeLibrary, 'IsCompositionActive'); // Do not localize
      if Assigned(_IsCompositionActive) then
        Result := _IsCompositionActive;
    end;
  end;
...
begin
  ...
  begin
    ...
    if ThemeLibrary > 0 then
    begin
      _GetThemeTransitionDuration := GetProcAddress(ThemeLibrary, 'GetThemeTransitionDuration'); // Do not localize
      if Assigned(_GetThemeTransitionDuration) then
        Result := _GetThemeTransitionDuration(hTheme, iPartId, iStateIdFrom,
          iStateIdTo, iPropId, pdwDuration);
    end;
