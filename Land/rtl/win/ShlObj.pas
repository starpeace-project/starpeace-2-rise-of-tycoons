...
{$HPPEMIT 'struct tagMACHINE_ID;'}
{$HPPEMIT 'struct tagDOMAIN_RELATIVE_OBJECTID;'}
{$HPPEMIT '#endif'}
...
{   BOOL fShowAllObjects : 1;
  ...
  {$EXTERNALSYM PShellFlagState}
  TShellFlagState = SHELLFLAGSTATE;
  {$EXTERNALSYM TShellFlagState}
...
type
  ...
  {$EXTERNALSYM tagMACHINE_ID}
  tagMACHINE_ID = packed record
    szName: packed array[0..15] of Char;
  end;
  {$EXTERNALSYM MACHINE_ID}
  MACHINE_ID = tagMACHINE_ID;
  PMachineID = ^TMachineID;
  TMachineID = tagMACHINE_ID;

  {$EXTERNALSYM tagDOMAIN_RELATIVE_OBJECTID}
  tagDOMAIN_RELATIVE_OBJECTID = packed record
    guidVolume: TGUID;
    guidObject: TGUID;
  end;
  {$EXTERNALSYM tagDOMAIN_RELATIVE_OBJECTID}
...
function SHCreateItemInKnownFolder(const kfid: TIID; dwKFFlags: DWORD;
  pszItem: LPCWSTR; const riid: TIID; out ppv): HResult;
...
function SHGetIDListFromObject(const punk: IUnknown; out ppidl: PItemIDList): HResult;
...
var
  _SHCreateItemFromIDList: function(pidl: PItemIDList; const riid: TIID;
    out ppv): HResult; stdcall;

  _SHCreateItemFromParsingName: function(pszPath: LPCWSTR; const pbc: IBindCtx;
    const riid: TIID; out ppv): HResult; stdcall;

  _SHCreateItemWithParent: function(pidlParent: PItemIDList;
    const psfParent: IShellFolder; pidl: PItemIDList; const riid: TIID;
    out ppvItem): HResult; stdcall;

  _SHCreateItemFromRelativeName: function(const psiParent: IShellItem;
    pszName: LPCWSTR; const pbc: IBindCtx; const riid: TIID; out ppv): HResult; stdcall;

  _SHCreateItemInKnownFolder: function(const kfid: TIID; dwKFFlags: DWORD;
    pszItem: LPCWSTR; const riid: TIID; out ppv): HResult; stdcall;

  _SHGetIDListFromObject: function(const punk: IUnknown; out ppidl: PItemIDList): HResult; stdcall;

  _SHGetPropertyStoreFromIDList: function(pidl: PItemIDList; flags: Integer;
    const riid: TIID; out ppv): HResult; stdcall;

  _SHGetPropertyStoreFromParsingName: function(pszPath: LPCWSTR;
    const pbc: IBindCtx; flags: Integer; const riid: TIID; out ppv): HResult; stdcall;

  _SHGetNameFromIDList: function(pidl: PItemIDList; sigdnName: Integer;
...
function SHCreateItemWithParent(pidlParent: PItemIDList;
  ...
  out ppvItem): HResult;
begin
  if Assigned(_SHCreateItemWithParent) then
    Result := _SHCreateItemWithParent(pidlParent, psfParent, pidl, riid, ppvItem)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHCreateItemWithParent := GetProcAddress(Shell32Lib, 'SHCreateItemWithParent'); // Do not localize
      if Assigned(_SHCreateItemWithParent) then
        Result := _SHCreateItemWithParent(pidlParent, psfParent, pidl, riid,
          ppvItem);
    end;
  end;
end;
...
function SHCreateItemFromRelativeName(const psiParent: IShellItem;
  pszName: LPCWSTR; const pbc: IBindCtx; const riid: TIID; out ppv): HResult;
begin
  if Assigned(_SHCreateItemFromRelativeName) then
    Result := _SHCreateItemFromRelativeName(psiParent, pszName, pbc, riid, ppv)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHCreateItemFromRelativeName := GetProcAddress(Shell32Lib, 'SHCreateItemFromRelativeName'); // Do not localize
      if Assigned(_SHCreateItemFromRelativeName) then
        Result := _SHCreateItemFromRelativeName(psiParent, pszName, pbc, riid,
          ppv);
    end;
  end;
end;
...
function SHCreateItemInKnownFolder(const kfid: TIID; dwKFFlags: DWORD;
  pszItem: LPCWSTR; const riid: TIID; out ppv): HResult;
begin
  if Assigned(_SHCreateItemInKnownFolder) then
    Result := _SHCreateItemInKnownFolder(kfid, dwKFFlags, pszItem, riid, ppv)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHCreateItemInKnownFolder := GetProcAddress(Shell32Lib, 'SHCreateItemInKnownFolder'); // Do not localize
      if Assigned(_SHCreateItemInKnownFolder) then
        Result := _SHCreateItemInKnownFolder(kfid, dwKFFlags, pszItem, riid, ppv);
    end;
  end;
...
function SHGetIDListFromObject(const punk: IUnknown; out ppidl: PItemIDList): HResult;
begin
  if Assigned(_SHGetIDListFromObject) then
    Result := _SHGetIDListFromObject(punk, ppidl)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHGetIDListFromObject := GetProcAddress(Shell32Lib, 'SHGetIDListFromObject'); // Do not localize
      if Assigned(_SHGetIDListFromObject) then
        Result := _SHGetIDListFromObject(punk, ppidl);
    end;
  end;
...
function SHGetPropertyStoreFromIDList(pidl: PItemIDList; flags: Integer;
  const riid: TIID; out ppv): HResult;
begin
  if Assigned(_SHGetPropertyStoreFromIDList) then
    Result := _SHGetPropertyStoreFromIDList(pidl, flags, riid, ppv)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHGetPropertyStoreFromIDList := GetProcAddress(Shell32Lib, 'SHGetPropertyStoreFromIDList'); // Do not localize
      if Assigned(_SHGetPropertyStoreFromIDList) then
        Result := _SHGetPropertyStoreFromIDList(pidl, flags, riid, ppv);
    end;
  end;
...
function SHGetPropertyStoreFromParsingName(pszPath: LPCWSTR;
  const pbc: IBindCtx; flags: Integer; const riid: TIID; out ppv): HResult;
begin
  if Assigned(_SHGetPropertyStoreFromParsingName) then
    Result := _SHGetPropertyStoreFromParsingName(pszPath, pbc, flags, riid, ppv)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL;
    if Shell32Lib > 0 then
    begin
      _SHGetPropertyStoreFromParsingName := GetProcAddress(Shell32Lib, 'SHGetPropertyStoreFromParsingName'); // Do not localize
      if Assigned(_SHGetPropertyStoreFromParsingName) then
        Result := _SHGetPropertyStoreFromParsingName(pszPath, pbc, flags, riid,
          ppv);
    end;
  end;
end;

function SHGetNameFromIDList(pidl: PItemIDList; sigdnName: Integer; var ppszName: LPWSTR): HResult;
begin
  if Assigned(_SHGetNameFromIDList) then
    Result := _SHGetNameFromIDList(pidl, sigdnName, ppszName)
  else
  begin
    InitShlObj;
    Result := E_NOTIMPL; 
    if Shell32Lib > 0 then
    begin
      _SHGetNameFromIDList := GetProcAddress(Shell32Lib, 'SHGetNameFromIDList'); // Do not localize
      if Assigned(_SHGetNameFromIDList) then
        Result := _SHGetNameFromIDList(pidl, sigdnName, ppszName);
    end;
  end;
