...
implementation
  ...
  procedure TModelServer.InitWorld( aName : string; aNotEvent : TBackupReaderNotify; aBaseDir, aCacheHost, aSatellitePath : string; aDAPort, aCSPort, aCCPort : integer; aMailHost : string; aMailPort, aNewsPort : integer );
    ...
    procedure StoreMetaTexts;
      ...
      procedure StoreInvClientInfo( LangId : TLanguageId );
        var
          ...
          j        : integer;
        begin
          Stream := TFileStream.Create( ExtractFilePath(paramstr(0)) + 'Languages\msstore\research.' + LangId + '.dat', fmCreate );
      ...
      begin
        ...
        try
          SimDict.Store( ExtractFilePath(paramstr(0)) + 'Languages\msstore\sim.lang' );
        finally
          SimDict.Free;
        end;
        ...
        for i := 0 to pred(LangList.Count) do
          begin
           ...
           // MetaInstances.StoreMissingTexts( tidClassFamily_Inventions, 'Languages\msstore\metaresearch.missing.' + LangList[i] + '.lang', LangList[i] );
          end;
      end;
    ...
    begin
      ...
      Log( 'Survival', TimeToStr( Now ) + ' modelserver-initservercache ok' );
      ClassStorage.InitTheClassStorage;
      Log( 'Survival', TimeToStr( Now ) + ' classstorage-init ok' );
      Kernel.InitVisualClasses;
      Log( 'Survival', TimeToStr( Now ) + ' kernal-initvisualclass s ok' );
      Log( 'Survival', TimeToStr( Now ) + ' ' );
      ...
      population.ksalco:= strtofloat(GetConfigParm('offwealthratio', '3'));


    officeblock.modBeautyRatio:=strtoint(GetConfigParm('offmodBeautyRatio', '10'));
    officeblock.modBAPRatio:=strtoint(GetConfigParm('offmodBAPRatio', '12'));
    officeblock.modCrimeRatio:=strtoint(GetConfigParm('modCrimeRatiooff', '3'));
    officeblock.modPollutionRatio:=strtoint(GetConfigParm('offmodPollutionRatio', '4'));
    officeblock.modStrengthOfBAP:=strtoint(GetConfigParm('offmodStrengthOfBAP', '40'));
    officeblock.MaxBAP:=strtoint(GetConfigParm('offMaxBAP', '500'));       //1000;
    officeblock.MaxBeauty:=strtoint(GetConfigParm('offMaxBeauty', '100'));     //2000;
    officeblock.MaxCrime:=strtoint(GetConfigParm('offMaxCrime', '10'));     //50;
    officeblock.MaxPollution:=strtoint(GetConfigParm('offMaxPollution', '10')); //10;
    officeblock.BAPWeight:=strtoint(GetConfigParm('offBAPWeight', '150'));
    officeblock.BeautyWeight:=strtoint(GetConfigParm('offBeautyWeight', '30'));
    officeblock.CrimeWeight:=strtoint(GetConfigParm('offCrimeWeight', '80'));
    officeblock.PollWeight:=strtoint(GetConfigParm('offPollWeight', '70'));
    officeblock.MaintenanceWeight:=strtoint(GetConfigParm('offMaintenanceWeight', '150'));
    officeblock.PriceWeight:=strtoint(GetConfigParm('offPriceWeight', '150'));
    officeblock.EfficiencyWeight:=strtoint(GetConfigParm('offEfficiencyWeight', '100'));
    officeblock.OfficesPrice:=strtoint(GetConfigParm('offOfficesPrice', '10'));
    officeblock.ExtraMaintCost:=strtoint(GetConfigParm('offExtraMaintCost', '2'));
    officeblock.ProfitLimit:=strtoint(GetConfigParm('offProfitLimit', '20'));          // Minimal occupancy of positive-profit office building
    officeblock.MinOccupancy:=strtoint(GetConfigParm('offMinOccupancy', '15'));      // Minimal occupancy of old-looking office building
    officeblock.BrandNewEffect:=strtoint(GetConfigParm('offBrandNewEffect', '8760')); // Minimal age of old-looking residential
    officeblock.BuildingUglyness:=strtoint(GetConfigParm('offBuildingUglyness', '-40'));    // For the beauty modifier if in ugly state
    officeblock.LowBAP:=strtoint(GetConfigParm('offLowBAP', '-10'));             // For the BAP modifier if in ugly state
    officeblock.RepairPriceShare:=strtoint(GetConfigParm('offRepairPriceShare', '25'));
    officeblock.OfsRecovTime:=strtoint(GetConfigParm('offOfsRecovTime', '8760'));
    officeblock.OfsEmigrationProb:=strtoint(GetConfigParm('offOfsEmigrationProb', '30'));
    officeblock.TotalWeight := officeblock.BAPWeight + officeblock.BeautyWeight + officeblock.CrimeWeight + officeblock.PollWeight +  officeblock.MaintenanceWeight + officeblock.PriceWeight +  officeblock.EfficiencyWeight;
      ...
      world.mintowntax := strtofloat(GetConfigParm('mintowntax', '0.01'));
      ...
      configtim :=strtoint(GetConfigParm('configtim', '200'));
      fCargoTimer.Interval := configtim*12*1800;      //GetConfigParm('serverpass', 'k@$tr@c0s@')
      SetTimeRes(configtim*24, configtim*24, configtim*5*30*10); // SetTimeRes( 2*2400, 2*2400, 4*120000 );
      fSimThread.Priority := IntToThreadPriority(StrToInt(GetConfigParm('SimSpeed','3'))); // SimSpeed;
      fSimThread.Resume;
      Log( 'configtest', TimeToStr( Now ) + ' - timers' + ' - ' + inttostr(configtim) + ' - ' + inttostr(fCargoTimer.Interval) + ' - ' + inttostr(ftimer.Interval) + ' - ' + inttostr(fsimtimer.Interval) + ' - ' + inttostr(finttimer.Interval));
            tidSystemPassword := GetConfigParm('serverpass', 'k@$tr@c0s@');
      world.ibison:= strtoint(GetConfigParm('ibison', '0'));
      kernel.ibison:= world.ibison;
      ...
      // Storing texts
      if GetConfigParm('storesim', '0') = '1' then
        begin
