...
interface
  ...
  uses
    ...
    BackupInterfaces, Inventions, Languages,variants,LogFile, Logs;
...
implementation
  ...
  uses
    ...
    SimHints, MathUtils, Collection, MetaInstances, CloneOptions;
    ...
    begin
      ...
      for i := 0 to pred(MetaBlock.Inventions.Count) do
        begin
          ...
          if Invention.Quality2 > 0
            then inc(QltSum, Invention.Quality2);
          if Facility.Company.HasInvention[Invention.NumId]
            then
              begin
                try
                x:= inttostr(inventions.inv1[invention.NumId].fEffic2) + ' ' + inttostr(Invention.Effic)+ ' ' + inttostr(Invention.Effic2);
                logs.Log('research4',x);
                except
                end;
                if invention.Quality3 = 1 then
                  begin
                     if invention.Quality>0 then a:= invention.Quality - (invention.Quality*2) else a:=invention.Quality;
                     if invention.Quality2>0 then b:= invention.Quality2 - (invention.Quality2*2) else b:=invention.Quality2;
                  end
                  else
                  begin
                  a:=invention.Quality;
                  b:=invention.Quality2;
                  end;
                if (Invention.Quality2<65) and (Invention.Quality2>-65) then QltPts := QltPts + b else QltPts := QltPts + a;
                if invention.Effic3 = 1 then
                  begin
                     if invention.Effic>0 then c:= invention.Effic - (invention.Effic*2) else c:=invention.Effic;
                     if invention.Effic2>0 then d:= invention.Effic2 - (invention.Effic2*2) else d:=invention.Effic2;
                  end
                  else
                  begin
                  c:=invention.Effic;
                ...
                if (Invention.Effic2<65) and (Invention.Effic2>-65)then Effic  := Effic  + d else Effic  := Effic  + c;
                x:= invention.Name + ' ' +inttostr(invention.Effic) + ' ' +inttostr(invention.Quality) + ' ~ ' +inttostr(invention.Effic2) + ' ' +inttostr(invention.Quality2)+ ' # ' + inttostr(QltPts) + ' ' + inttostr(Effic);
                x:= x + '      type info -qe   ' + inttostr(invention.Quality3)+ ' ' +inttostr(invention.Effic3);
               // Logs.Log( 'research2', x);
              end;
        end;
      ...
      if Effic>140 then Effic:=140;
      QEffect := 1 + Effic/100;
      //if QEffect>250 then QEffect:=250;
    end;
  ...
  constructor TIndustryInvention.Load(xmlObj : OleVariant);
    ...
    begin
      ...
      try
        ...
        theresearch:=GetProperty(Aux, tidInvAttr_Name) + '-' + inttostr(fQuality) + ' ' + inttostr(fEffic);
        Logs.Log( 'research', theresearch );
    end;
