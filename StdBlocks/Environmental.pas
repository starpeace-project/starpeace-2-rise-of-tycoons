...
interface
  ...
  uses
    Classes, Collection, Kernel, Surfaces, BackupInterfaces, Accounts, BasicAccounts, mathutils;
...
implementation
  ...
  constructor TMetaEnvironmentalBlock.Create( anId : string; aBlockClass : CBlock );
    begin
      ...
      MaxUpgrade     := StrToInt(TheGlobalConfigHandler.GetConfigParm('MaxEnviromentUpgrade', '20'));
  ...
  procedure TMetaEnvironmentalBlock.RegisterModifier( Surface : TSurfaceId; Value, Strength : TSurfaceValue );
    ...
    begin
      ...
      ModifierInfo.fStrengthorg := Value;
  ...
  function TEnvironmentalBlock.Evaluate : TEvaluationResult;
    ...
    begin
      ...
      if not Facility.CriticalTrouble
        then
          begin
           BlockGenMoney(-dt*UpgradeLevel*TMetaEnvironmentalBlock(MetaBlock).MaintCost, accIdx_Public);
           //try out
               ...
               for i := 0 to pred(fmodifiers.Count) do
                begin
                   with TModifierInfo(fmodifiers[i]) do
                    begin
                       fvalue := uprlv*fstrengthorg;
                       TSurfaceModifier(self.fModifiers[i]).Value := fvalue;
                    end;
                end;
            end;
          end;
