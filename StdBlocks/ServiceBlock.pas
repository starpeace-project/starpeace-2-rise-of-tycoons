...
interface
  ...
  uses
    ...
    BackupInterfaces, Accounts, CacheAgent, Inventions, Languages,variants;
...
implementation
  ...
  constructor TMetaService.Create(anId, Name     : string;
    ...
    begin
      ...
      fBuyuniver := StrToFloat(TheGlobalConfigHandler.GetConfigParm('comdemand', '1'));
      // Set buy probabilities
      for kind := low(kind) to high(kind) do
        begin
        fBuyProb[kind] := fBuyuniver*fBuyBoost*aBuyProb[ord(kind)];
        logs.Log('servicebuy',afluidname + ' ' + floattostr(fBuyProb[kind]));
        end;
  ...
  function TMetaService.GetBuyProb(kind : TPeopleKind) : TBuyProbability;
    begin
      result := fBuyProb[kind]*fRecFact;   //         to do with relay economy, add back if needed
    end;                                   
  ...
  procedure TMetaServiceBlock.DeclareInvention(Invention : TInvention);
    ...
    begin
      ...
      //logs.log('r3',invention.Kind);
      //logs.log('r3',invention.ClassType.ClassParent.ClassParent.ClassName);
      if invention.kind = 'ServiceFacilities'//ObjectIs('TServiceBlockInvention', Invention)
        then
          begin
            Inv := TServiceBlockInvention(Invention);
            if Inv.Efficiency2 > 0
              then inc(fMaxEfficiency, Inv.Efficiency2);
            if Inv.Efficiency2 < 0
              then inc(fMinEfficiency, Inv.Efficiency2);
            if Inv.Desirability2 > 0
              then inc(fMaxDesirability, Inv.Desirability2);
            if Inv.Desirability2 < 0
              then inc(fMinDesirability, Inv.Desirability2);
            //logs.Log('r2', inttostr(fMaxEfficiency) + ' - ' + inttostr(fMinEfficiency));
          end;
    end;
       ...
       if not Facility.CriticalTrouble
        then
          begin
            ...
            //test for fix
            if fShowEfficiency<30 then fShowEfficiency:=30;
            if fShowEfficiency>100 then fShowEfficiency:=100;
...

{
  ...
  procedure TServiceBlock.IntegrateInventions(out invEfficiency, invDesirability : integer);
    ...
    begin
     invEfficiency   := 0;
      invDesirability := 0;
      //Logs.Log( 'research2', ' the start: ' + inttostr(invEfficiency) + ' ' + inttostr(invDesirability));
      for i := 0 to pred(MetaBlock.Inventions.Count) do
        begin
          ...
          if Facility.Company.HasInvention[Invention.NumId]
            then
              begin
                invEfficiency   := invEfficiency   + Invention.Efficiency2;
                invDesirability := invDesirability + Invention.Desirability2;
                //invention.Efficiency2
                //Logs.Log( 'research2', invention.Name + ' ' +inttostr(invention.Efficiency) + ' ' +inttostr(invention.Desirability) + ' # ' + inttostr(invEfficiency) + ' ' + inttostr(invDesirability) + ' ----- ' +inttostr(invention.Efficiency2));
              end;
        end;
      ...
      for i := 0 to pred(MetaBlock.Inventions.Count) do
        begin
          ...
          if Facility.Company.HasInvention[Invention.NumId]
            then
              begin
                if invention.Efficiency3 = 1 then
                  begin
                     if invention.Efficiency>0 then c:= invention.Efficiency - (invention.Efficiency*2) else c:=invention.Efficiency;
                     if invention.Efficiency2>0 then d:= invention.Efficiency2 - (invention.Efficiency2*2) else d:=invention.Efficiency2;
                  end
                  else
                  begin
                    c:=invention.Efficiency;
                    d:=invention.Efficiency2;
                  end;
                if (Invention.Efficiency2>-60) and (Invention.Efficiency2<60) then   invEfficiency:= invEfficiency   + d else invEfficiency:= invEfficiency   + c;
                if invention.Desirability3 = 1 then
                  begin
                     if invention.Desirability>0 then a:= invention.Desirability - (invention.Desirability*2) else a:=invention.Desirability;
                     if invention.Desirability2>0 then b:= invention.Desirability2 - (invention.Desirability2*2) else b:=invention.Desirability2;
                  end
                  else
                  begin
                    a:=invention.Desirability;
                     b:=invention.Desirability2;
                  end;
                if (Invention.Desirability2>-60) and (Invention.Desirability2<60)then invDesirability:= invDesirability + b else invDesirability:= invDesirability + a;
              //  Logs.Log( 'research2', invention.Name + ' ' +inttostr(invention.Efficiency) + ' ' +inttostr(invention.Desirability) + ' # ' + inttostr(invEfficiency) + ' ' + inttostr(invDesirability));
              end;
        end;

    if invDesirability>60 then invDesirability:=60; }
    //Logs.Log( 'research2', ' the end: ' + inttostr(invEfficiency) + ' ' + inttostr(invDesirability));
    end;
  ...
  function TServiceBlock.CalcEfficiencyPerc(value : single) : TPercent;
    ...
    begin
      ...
      //logs.Log('r1', floattostr(max) + ' - ' + floattostr(min));
      if max = min then max:= max +1/100;
