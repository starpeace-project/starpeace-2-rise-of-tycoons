...
interface
  ...
  uses
    ...
    Accounts, WorkCenterBlock, StdFluids, Inventions, Languages, MediaGates,variants;

  const
    tidInventionClass_MovieStudios = 'StudioReseach';
    {$ifdef newpack}
    bidGate_Chemicals = tidGate_Timber;
    bidFluid_Chemicals = tidFluid_Timber;
   {$ELSE}
   bidGate_Chemicals = tidGate_Chemicals;
    bidFluid_Chemicals = tidFluid_Chemicals;
   {$endif}
...
implementation
  ...
  uses
    ...
    Construction, StdAccounts, Logs, Standards, ModelServerCache,
    MediaNameGenerator;
    ...
    begin
      ...
      MetaInputs.Insert(
        TMetaInput.Create(
          ...
          qIlimited,
          TPullInput,
          TMetaFluid(TheClassStorage.ClassById[tidClassFamily_Fluids, bidFluid_Chemicals]),
          5,
          mglBasic,
  ...
  procedure TMovieStudios.AutoConnect(loaded : boolean);
    begin
      inherited;
      fChemInput   := InputsByName[bidFluid_Chemicals];
                  ...
                  if (Facility.Town <> nil) and (Facility.Town.WorldLocator <> nil)
                    then
                      begin
                        ...
                        if TotalPop > 0
                          then ExpSlope := 25*fReleaseFilm.fExpositions/TotalPop
                          else ExpSlope := 0;
