...
interface
  ...
  uses
    ...
    Sessions, GMKernel, ISMLS, Languages, RDOConnectionPool,variants,StrUtils;
  ...
  const
    ...
    DATimeOut             = 60*1000; // timeout = 10 sec!
    ...
    TClientView =
      class
        ...
        published
          ...
          // new
          function  buildkind(id:widestring) :olevariant;
          function  buildlist(id:widestring;clusid,kindid:integer) :olevariant;
          function  currinfo  :olevariant;
          function  bankinfo  :olevariant;
          function  plinfo  :olevariant;
          function  bankloan(amount:widestring)  :olevariant;
          function  resetaccount : olevariant;
          function  Advancelevel(i2:integer)   : olevariant;
          function  bankloanpay(amount:integer)  :olevariant;
          function  capitol  :olevariant;
          function  gettowns  :olevariant;
          function  banksend(usr,amt:widestring)  :olevariant;
          function  getranklist  :olevariant;
          function  getrank(rid:widestring)  :olevariant;
    ...
    TInterfaceServer =
      class
        ...
        private
          ...
          function CheckUserban(UserName : string) : boolean;
...
implementation
  ...
  uses
    ...
    DirectoryServerProtocol, Logs, cache;
  ...
  function  TClientView.getrank(rid:widestring)  :olevariant;
    var
        usr1 : tstringlist;
        i:integer;
    begin
        i:= cache.renewobject(rid + 'object.five');
        usr1 := tstringlist.Create;
        usr1.Text := cache.getfile(rid + 'object.five' );
        result:=usr1.Text;
        usr1.Free;
    end;

  function  TClientView.getranklist  :olevariant;
    begin
      result:= cache.trankings.Text;
    end;

  function  TClientView.banksend(usr,amt:widestring)  :olevariant;
    var
        usr1 : tstringlist;
        y,z:integer;
      begin
        //advance level
        usr1 := tstringlist.Create;
      z:= cache.RenewCache('Tycoon', username);
      usr1.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      y:= strtoint(usr1.Values['ObjectId']);
        result:= cache.banksend(usr,amt,y);
        usr1.Free;
      end;

  function  TClientView.gettowns  :olevariant;
    var
      output,towns :tstringlist;
      list1: tstrings;
      listp: string;
      i,z:integer;
    begin
      ...
      listp := cache.tcache + 'worlds\' + tworld + '\towns\';
      list1 := getsubdirectories(listp);
      for i := 0 to List1.Count - 1 do
        begin
           z:=cache.RenewCache('Town', StringReplace(list1.Strings[i], '.five','',[rfReplaceAll, rfIgnoreCase]));
           towns.Text:=  cache.getfile('towns/' + list1.Strings[i] + '/object.five' );
           output.Add('tname' + inttostr(i) + '=' + towns.Values['Name'] ); //output.Add('' + inttostr(i) + '=' + towns.Values[''] );
           output.Add('x' + inttostr(i) + '=' + towns.Values['xpos'] );
           output.Add('y' + inttostr(i) + '=' + towns.Values['ypos'] );
           output.Add('vid' + inttostr(i) + '=' + towns.Values['visualid'] );  //hiPopulation
           output.Add('hpop' + inttostr(i) + '=' + towns.Values['hiPopulation'] );
           output.Add('mpop' + inttostr(i) + '=' + towns.Values['midPopulation'] );
           output.Add('lpop' + inttostr(i) + '=' + towns.Values['loPopulation'] );
        end;
      output.Add('count' + '=' + inttostr(List1.Count) );
  ...
  function  TClientView.capitol  :olevariant;
    var
      output,usr : tstringlist;
    begin
      output := tstringlist.Create;
      usr := tstringlist.Create;
      usr.Text := cache.getfile('world.five' );
      if usr.Values['HasCapitol']='1' then
        begin
          output.Add('capitol' + '=1' );
          output.Add('capitolx' + '=' + usr.Values['CapitolX'] );
          output.Add('capitoly' + '=' + usr.Values['CapitolY']);
        end
        else output.Add('capitol' + '=0' );
      result:=output.Text;
      output.Free ;
      usr.Free;
    end;

  function  TClientView.buildlist(id:widestring;clusid,kindid:integer) :olevariant;
    var
      output : tstringlist;
      usr,cpany : tstringlist;
      i,j,z : integer;
      canbuild:boolean;
    begin
      //build list
      ...
      usr := tstringlist.Create;
      z:=cache.RenewCache('Tycoon', username);
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      cpany := tstringlist.Create;
      z:=cache.RenewCache('Company', id);
      cpany.Text := cache.getfile('companies\' + id + '.five\object.five' );
      j:=0;//cache.tcluster[clusid].count
      for i := 0 to cache.tcluster[clusid][kindid].count - 1 do
        begin              //output.Add('' + inttostr(i) + '=' + );
          canbuild:=false;
          if cache.tcluster[clusid][kindid].kinds[i].list.Values['InTown'] = '1'  then
           begin
              if cache.tcluster[clusid][kindid].kinds[i].list.Values['TechnologyName'] = '' then canbuild:= true else canbuild:=AnsiContainsStr(cpany.Values['Technology'],cache.tcluster[clusid][kindid].kinds[i].list.Values['TechnologyName']);
              if strtoint(cache.tcluster[clusid][kindid].kinds[i].list.Values['TypicalVisualClass']) = 8102 then
                begin
                  if strtoint(usr.Values['CurrLevel']) < 5 then canbuild:=false;
                end;
              if strtoint(cache.tcluster[clusid][kindid].kinds[i].list.Values['TypicalVisualClass']) = 8092 then
                begin
                  if strtoint(usr.Values['CurrLevel']) <> 4 then canbuild:=false;
                end;
              output.Add('canbuild' + inttostr(j) + '=' + booltostr(canbuild));
              output.Add('tvc' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['TypicalVisualClass']);
              output.Add('ztype' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['ZoneType']);
              output.Add('price' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Price']);
              output.Add('size' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Size']);
              output.Add('id' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Id']);
              output.Add('requires' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Requires' + flangid]);
              output.Add('desc' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Desc' + flangid]);
              output.Add('name' + inttostr(j) + '=' + cache.tcluster[clusid][kindid].kinds[i].list.Values['Name' + flangid]);
              j:=j+1;
            end;
        end;
      output.Add('listcount=' + inttostr(j));
      result:=output.Text;
      output.Free ;
      usr.Free;
      cpany.Free;
    ...
    function  TClientView.bankloan(amount:widestring)  :olevariant;
      var
        usr : tstringlist;
        y,z:integer;
      begin
      ...
      z:= cache.RenewCache('Tycoon', username);
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      y:= strtoint(usr.Values['ObjectId']);
        result:= cache.bankloanget(amount,y);
        usr.Free;
      end;

   function  TClientView.bankloanpay(amount:integer)  :olevariant;
      var
        usr : tstringlist;
        y,z:integer;
      begin
      ...
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      y:= strtoint(usr.Values['ObjectId']);
        result:= cache.bankloanpay(amount,y);
        usr.Free;
      end;

    function  TClientView.plinfo  :olevariant;
      var
      i,z,j,rcount:integer;
      output,output2 : tstringlist;
      usr : tstringlist;
      curritemlist : tstrings;
    begin
      output := tstringlist.Create;
      usr := tstringlist.Create;
      z:= cache.RenewCache('Tycoon', username);
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      output.Add('AccountCount=' + usr.Values['AccountCount']);
      for I := 0 to strtoint(usr.Values['AccountCount']) - 1 do
        begin
           {
           AccountHistory181=12,-10258917,0,82,86,84,79,76,74,76,74,72,67,62,50,
AccountIsTax181=1
AccountLevel181=3
AccountSecValue181=0
AccountValue181=-8202061452.8201
AccountName181.0=Offices
AccountId181=384
}
           output.Add('AccountHistory' + inttostr(i) + '=' + usr.Values['AccountHistory'+ inttostr(i)]);
           output.Add('AccountIsTax' + inttostr(i) + '=' + usr.Values['AccountIsTax'+ inttostr(i)]);
           output.Add('AccountLevel' + inttostr(i) + '=' + usr.Values['AccountLevel'+ inttostr(i)]);
           output.Add('AccountSecValue' + inttostr(i) + '=' + usr.Values['AccountSecValue'+ inttostr(i)]);
           output.Add('AccountValue' + inttostr(i) + '=' + usr.Values['AccountValue'+ inttostr(i)]);
           output.Add('AccountName' + inttostr(i) + '=' + usr.Values['AccountName'+ inttostr(i)+'.'+flangid]);
           output.Add('AccountId' + inttostr(i) + '=' + usr.Values['AccountId'+ inttostr(i)]);
        end;
    ...
    function  TClientView.bankinfo  :olevariant;
    var
      i,z,j,rcount:integer;
      output,output2 : tstringlist;
      usr : tstringlist;
...
LoanDate0=5732664
LoanTerm0=160
LoanSlice0=50251.2563
LoanAmount0=8040201.0043
LoanInterest0=0
LoanBankName0=Bank of IFEL
LoanCount=4  }
      output := tstringlist.Create;
      usr := tstringlist.Create;
      z:= cache.RenewCache('Tycoon', username);
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      output.Add('Budget=' + usr.Values['Budget']);
      output.Add('IFELLoanEstimated=' + usr.Values['IFELLoanEstimated']);
      output.Add('LoanAmount=' + usr.Values['LoanAmount']);
      output.Add('LoanCount=' + usr.Values['LoanCount']);
      for I := 0 to strtoint(usr.Values['LoanCount']) - 1 do
        begin
          output.Add('PayDate' + inttostr(i) + '=' + usr.Values['PayDate'+ inttostr(i)]);
          output.Add('LoanDate' + inttostr(i) + '=' + usr.Values['LoanDate'+ inttostr(i)]);
          output.Add('LoanTerm' + inttostr(i) + '=' + usr.Values['LoanTerm'+ inttostr(i)]);
          output.Add('LoanSlice' + inttostr(i) + '=' + usr.Values['LoanSlice'+ inttostr(i)]);
          output.Add('LoanAmount' + inttostr(i) + '=' + usr.Values['LoanAmount'+ inttostr(i)]);
          output.Add('LoanInterest' + inttostr(i) + '=' + usr.Values['LoanInterest'+ inttostr(i)]);
          output.Add('LoanBankName' + inttostr(i) + '=' + usr.Values['LoanBankName'+ inttostr(i)]);
        end;
     //send data
      result:=output.Text;
      output.Free;
   ...
   function  TClientView.currinfo  :olevariant;
    var
      i,z,j,rcount:integer;
      output,output2 : tstringlist;
      usr : tstringlist;
    ...
    begin
      ...
      z:= cache.RenewCache('Tycoon', username);
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      //load info into output       output.Add('xx=' + usr.Values['']);        + flangid
      output.Add('ProfitAverage=' + usr.Values['ProfitAverage']);
      output.Add('AdvanceToNextLevel=' + usr.Values['AdvanceToNextLevel']);
      output.Add('Budget=' + usr.Values['Budget']);
      output.Add('Prestige=' + usr.Values['Prestige']);
      output.Add('Nobility=' + usr.Values['Nobility']);
      //level part:    LevelDesc  LevelName  CurrLevel   NextLevelCond
      output.Add('CurrLevel=' + usr.Values['CurrLevel']);
      output.Add('LevelName=' + usr.Values['LevelName' + flangid]);
      output.Add('LevelDesc=' + usr.Values['LevelDesc' + flangid]);
      output.Add('NextLevelCond=' + usr.Values['NextLevelCond' + flangid]);
      output.Add('NextLevelDesc=' + usr.Values['NextLevelDesc' + flangid]);
      output.Add('NextLevelName=' + usr.Values['NextLevelName' + flangid]);
      //Rankings
      rcount:= strtoint(usr.Values['RankCount']) ;
      j:=0;
      for i := 0 to rcount - 1 do
        begin
          if strtoint(usr.Values['RankPos' + inttostr(i)]) < 90000 then
            begin
               output.Add('RankName' + inttostr(j) + '=' + usr.Values['RankName' + inttostr(i) + '.' + flangid ]);
               output.Add('RankPos' + inttostr(j) + '=' + usr.Values['RankPos' + inttostr(i)]);
               j:=j+1;
            end;
        end;
      output.Add('RankCount=' + inttostr(j));
      // curr items
      curritemlist := tstringlist.Create;
      curritemlist:= cache.GetFilenames('tycoons\' + username + '.five\Curriculum\') ;
      Logs.Log('e1', 'curr 1 ' +inttostr(curritemlist.Count) );
      output2 := tstringlist.Create;
      for i := 0 to curritemlist.Count - 1 do
        begin
          Logs.Log('e1', 'curr 1.1 ' + curritemlist.Strings[i] );
          output2.Text :=cache.getfile('tycoons\' + username + '.five\Curriculum\' + curritemlist.Strings[i] );
          Logs.Log('e1', 'curr 2' );
          output.Add('curDesc' + inttostr(i) + '=' + output2.Values['Desc' + flangid]);
          Logs.Log('e1', 'curr 3' );
          output.Add('curPrestige' + inttostr(i) + '=' + output2.Values['Prestige']);
          Logs.Log('e1', 'curr 4' );
        end;
        
      output.Add('curritemcount=' + inttostr(curritemlist.Count));
      //output.Add('xx=' + usr.Values['']);
      //send data
      result:=output.Text;
      output.Free;
      output2.Free;
    ...
    function  TClientView.resetaccount : olevariant;
    var
      ...
      x1:olevariant;
      begin
        //TInterfaceServer.worldproxy.RDOResetTycoonEx(UserName,Password);

        //reset user account
        fServer.WorldProxy.RDOResetTycoonEx(UserName,Password);
        //result:= cache.resetaccount(UserName,Password);
      {   efWSISCnx      := TWinSockRDOConnection.Create('mscac4');
       ...
       logs.log('rset','connection5 ' + UserName + '-' + Password);
       // do reset             widestring(usr),widestring(passw)
           x1:=efISProxy.RDOResetTycoonEx(UserName,Password);
    ...
    function  TClientView.Advancelevel(i2:integer)   : olevariant;
      var
        usr : tstringlist;
        y,z:integer;
      begin
      ...
      usr.Text := cache.getfile('tycoons\' + username + '.five\object.five' );
      y:= strtoint(usr.Values['ObjectId']);
        result:= cache.advancelevel(i2,y);
        usr.Free;
      end;
   ...
   function  TClientView.buildkind(id:widestring) :olevariant;
    var
      output : tstringlist;
      usr : tstringlist;
      canzone,canroad : boolean;
      a,i,j,z : integer;
    begin
      //get company kind list
      if id='Dissident' then id:='Dissidents';
      logs.Log('buildmenu','startkind - 1' + id);
      output := tstringlist.Create;
      usr := tstringlist.Create;
      ...
      //usr.SaveToFile('build.txt');
      //can build roads,  can zone
      canroad := false;
      canzone := false;
      logs.Log('buildmenu','startkind - 3.1');
      if (strtoint(usr.Values['CurrLevel']) > 0) or (strtoint(usr.Values['Rolescount']) > 0)then canroad := true;
      logs.Log('buildmenu','startkind - 3.2');
      output.Add('canroad=' + booltostr(canroad));
      logs.Log('buildmenu','startkind - 3.3');
      if (strtoint(usr.Values['Rolescount']) > 0)then canzone := true;
      if usr.Values['IsMayor']='1' then canzone := true;
      if usr.Values['IsMinister']='1' then canzone := true;
      if usr.Values['IsCapitalMayor']='1' then canzone := true;
      logs.Log('buildmenu','startkind - 3.4');
      output.Add('canzone=' + booltostr(canzone));
      logs.Log('buildmenu','startkind - 4');
      //company kind list
      for i := 0 to 20 do    //cluster id
        begin
          if id = cache.tclustername[i] then a := i;
        end;
      logs.Log('buildmenu','startkind - 5 - ' + inttostr(a));
      output.Add('clusid='+ inttostr(a) ); //clusterid = a
      output.Add('clusname='+ id );
      output.Add('cluscount=' + inttostr(cache.tclustercount[a]) );
      logs.Log('buildmenu','startkind - 6');
      for j := 0 to cache.tclustercount[a] - 1 do   // go though each kind
        begin
          //whats needed - kindid=j, name = 'name'+landid, 'id' for images
          output.add('kindid' + inttostr(j) + '=' + inttostr(j));
          output.add('name' + inttostr(j) + '=' + cache.tcluster[a][j].info.Values['Name' + flangid]);
          output.add('id' + inttostr(j) + '=' + cache.tcluster[a][j].info.Values['Id']);

        end;

      logs.Log('buildmenu','startkind - 7');
      //send result
      result:=output.Text;
      output.Free;
      usr.Free;
  ...
  function TClientView.NewCompany( name, cluster : widestring ) : OleVariant;
    begin
      if cluster='Dissident' then cluster:='Dissidents';
  ...
  constructor TInterfaceServer.Create( aMSAddress, MailAddr, aDSAddress, aGMAddress, aDSArea : string; aMSPort, aMSEventsPort, MailPort, aDSPort, aGMPort, aClientPort : integer; FixAddr, BanListFile : string );
    begin
      ...
      try
        ...
        fDAClientConn.Server := aMSAddress;       //     'localhost';//                       'localhost';//
...
*)
  ...
  function TInterfaceServer.CheckUserAccount(UserName, Password : string) : boolean;
    ...
    begin
      ...
      try
        ...
        if paidfor then
          begin
            if (res = DIR_NOERROR) then  result := true else result:= false;
          end
        else
         begin
            if (res = DIR_NOERROR_StillTrial) or (res = DIR_NOERROR) then  result := true else result:= false;
          end;
        // check banlist
        res2 := checkuserban(username);
        if res2 then  result:= false;
        logs.Log('paidforcheck', booltostr(paidfor) + ' ' + username + ' ' + inttostr(res) + ' ' + booltostr(result));
      except
        result := true;
      end;
    end;

  function TInterfaceServer.CheckUserban(UserName : string) : boolean;
    var
      res, i : integer;
    begin
      result := false;
      //  check the ban list   BanListFile      
       fBanList.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'BannedPlayers.txt');
       res:= 0;
       for I := 0 to fbanlist.Count - 1 do
        begin
          if uppercase(fbanlist[i]) = uppercase(username) then res:= 1;
          //logs.log('banlistcheck', fbanlist[i])
        end;
       if invban then
        begin
                ...
                if (TycoonProxyId <> 0)
                  then
                    begin
                      ...
                      Logs.Log('l1', '(Logon.5 2)');
                      //c1:=fDACnntPool.ConnetionCount;
                      CheckDAConnections;
                      Logs.Log('l1', '(Logon.5 2 1)');
                      Cnx := fDACnntPool.GetConnection;
                      if Cnx <> nil
                        then
                          begin
                            Logs.Log('l1', '(Logon.5 3)');
                            ClientView.fTycoonProxy.SetConnection(Cnx);
                            Logs.Log('l1', '(Logon.5 4)');
                            ClientView.fConnection := Cnx;
                            ClientView.fTycoonProxy.BindTo( TycoonProxyId );
                            Logs.Log('l1', '(Logon.5 5)');
                            //ClientView.fTycoonProxy.
                            try
                              begin
                              ...
                              begin
                                Logs.Log('l1', '(Logon.5 5 id error)');
                                RenewWorldProxy;
                                c1:=fDACnntPool.ConnetionCount;
                                for i := 0 to c1 - 1 do
                                  begin
                                    //redo all connections
                                    Logs.Log('l1', '(Logon.5 5 redo connections )' + inttostr(i));
                                    fDACnntPool.SetConnection(i, GetDAConnection);
                                    
                                  end;
                                Logs.Log('l1', '(Logon.5 5 set new connection for user)');
                                Cnx := fDACnntPool.GetConnection;
                                ClientView.fTycoonProxy.SetConnection(Cnx);
                                ClientView.fConnection := Cnx;
                                ClientView.fTycoonProxy.BindTo( TycoonProxyId );
                                Logs.Log('l1', '(Logon.5 5 bind to)');
                                ClientView.fTycoonId := ClientView.fTycoonProxy.Id;
                                Logs.Log('l1', '(Logon.5 5 new id - finnish)');
                              end;
                            end;
                            Logs.Log('l1', '(Logon.5 5b)- tid = ' + inttostr(ClientView.fTycoonId)); // 
                            ClientView.fUserName := UserName; //ClientView.fTycoonProxy.Name;
                            Logs.Log('l1', '(Logon.5 5c)');
                            ClientView.fRealName := ClientView.fTycoonProxy.RealName;
                            Logs.Log('l1', '(Logon.5 5d)');
                            ClientView.fLangId   := ClientView.fTycoonProxy.Language;
                            Logs.Log('l1', '(Logon.5 6)');
  ...
  function TInterfaceServer.CanJoinWorldEx(Name : widestring) : OleVariant;
    ...
    begin
      ...
      //max worlds fDSProxy
      logs.Log('canjoin','start ' + dsarea);
      if dsarea = 'america' then
        begin
          //get info from ds server, send world name as well
          x1:= fDSProxy.RDOCanJoinNewWorld2(name,worldname);
          logs.Log('canjoin','start ' + booltostr(x1));
          if x1=false then result:=-2;
          ...
          begin
            ...
            for i := 0 to 12 do
              begin
                ...
                Channel := TChannel.Create(langchan[i], langchan[i], '', '', '', 0);
