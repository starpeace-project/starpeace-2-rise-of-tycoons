...
type
  TInterfaceServerReport = class(TForm)
    ...
    //procedure paidforClick(Sender: TObject);
  private
  ...
  procedure TInterfaceServerReport.StartServerClick(Sender: TObject);
    ...
    begin
      try
        ...
        try
          logs.Log('startup','1');
          interfaceserver.paidfor := paidfor.Checked;
          interfaceserver.invban := invban.Checked;
          cache.tcache := cacheloc.Text;
          cache.tworld := worldname.Text;
          logs.Log('startup','2');
          ...
          logs.Log('startup','4');
          x1 := cache.loadcluster; //load cluster info
          logs.Log('startup','4.1');
          x1 := cache.loadrank; //load rank info
          logs.Log('startup','4.2');
          cache.tcbport := strtoint(cbport.text);
          cache.tcbport2 := strtoint(daPort.text)+1;
          logs.Log('startup','5');
          i:= cache.connectms(strtoint(cbport.text),daHostAddress.Text);
          logs.Log('startup','6');
        except
          ...
          if FindKey//Reg.OpenKey( ISKey + WorldNameS , false )
            then
              begin
                ...
                try
                  invban.Checked  := Reg.ReadBool( 'invban' );
                  paidfor.Checked  := Reg.ReadBool( 'paidfor' );
                  cacheloc.Text := Reg.ReadString( 'cacheloc' );
                  cbport.text :=  Reg.ReadString( 'cbport' );
                finally
                  // none
                end;
                if UseDS.Checked
                  ...
                  if DSProxy.RDOCreateFullPathKey( key, true )
                    then
                      begin
                        ...
                        if IB.Checked = true then  DSProxy.RDOWriteInteger( 'ib', 1 )
                          else  DSProxy.RDOWriteInteger( 'ib', 0 );
                          ...
                          if Reg.OpenKey( ISKey + WorldNameS, true )
                            then
                              begin
                                ...
                                Reg.WriteBool( 'invban', invban.Checked );
                                Reg.WriteBool( 'paidfor', paidfor.Checked );
                                Reg.WriteString( 'cacheloc', cacheloc.Text );
                                Reg.WriteString( 'cbport', cbport.Text );
                                if paramstr(2) = ''
