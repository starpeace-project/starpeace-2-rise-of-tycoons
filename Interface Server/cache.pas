...
interface
  ...
  //standard functions
    ...
    function renewobject(path:widestring):integer;
         ...
         for k := 0 to List2.Count - 1 do
           begin
              ...
              for l := 0 to List3.Count - 1 do
                begin
                  a3:=a2 + list3.strings[l] + '\';
                  cfile.LoadFromFile(a3 + 'object.five');     // level 0 NTA
                  trankings.Add('name' + inttostr(i) + '=' + cfile.Values['RankingName0']);
                  trankings.Add('path' + inttostr(i) + '=' + cfile.Values['Path']);
                  trankings.Add('level' + inttostr(i) + '=' + '3');
                  i:=i+1;
                  logs.Log('loadrank','7');
                end;
           end;
 ...
 function GetSubDirectories(const directory : string):TStrings ;
  ...
  function RenewCache(ctype, username:widestring):integer;
    ...
    begin
      ...
      //if not fISProxy.Alive then i:= connectms(tcbport,'localhost');
  ...
  function renewobject(path:widestring):integer;
    ...
    begin
        ...
        usr1.Text := cache.getfile(path);
        s1:=fISProxy.GetCache(strtoint(usr1.Values['ObjectId']),strtoint(usr1.Values['sobjKind']),strtoint(usr1.Values['sobjInfo']));
        result:=1;
        usr1.Free;
    end;
