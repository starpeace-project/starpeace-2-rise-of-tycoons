...
uses
  ...
  FramedButton, VisualControls, worldlogon,BackgroundHandlerViewer,
  ...
  ImgList, CustomWebBrowser,logs,wlogon;
...
implementation
  ...
  const
    ...
    tidWorldProp_Rulest      = 'Interface/rt';
    tidWorldProp_Nob      = 'Interface/nob';
    tidWorldProp_Reset      = 'Interface/Reset';
    tidWorldProp_IB      = 'Interface/ib';
  ...
  procedure TUniversalMapView.Start;
    ...
    begin
      ...
      DSAddr := 'starpeacelive.com';//ConfigHolder.ReadString( true, '', 'DSAddr', 'starpeacelive.com' );

      // Patch Exodus
      if (DSAddr = '') or (DSAddr = '64.69.182.122')
        then
          begin
            DSAddr := 'starpeacelive.com';
    ...
    begin
      try
        ...
        try
          ...
          DSCnx.Port   := 2222;//fDSPort;
          DSProxy      := TRDOObjectProxy.Create as IDispatch;
          //logs.Log('universe','s - ' + fdsaddr + ' - ' + inttostr(fdsport));
          //logs.Log('universe','1');
                    ...
                    begin
                      try
                        ...
                        try
                          ...
                          props.Add( tidWorldProp_Rulest );
                          props.Add( tidWorldProp_Nob );
                          props.Add(  tidWorldProp_Reset );
                          props.Add(  tidWorldProp_IB );
                          WorldList.Text := DSProxy.RDOQueryKey( key, props.text );
                          //logs.Log('universe','4' + WorldList.Text);
                        finally
                          props.Free;
                        end;
        ...
        //logs.Log('universe','5');
          ...
          //logs.Log('universe','6-' + inttostr(count));
          if count=0
            then
              //logs.Log('universe','11')//Join(syncGetInfoHTML, [Area] )
            else
              begin
                //logs.Log('universe','9');
                WorldList.Values['Area'] := Area;
                //logs.Log('universe','10');
                    ...
                    for i := 0 to pred(count) do
                      with WorldList.Items.Add do
                        begin
                          ...
                          j:= 4 * strtoint(Worlds.Values[tidWorldProp_IB + IntToStr(i)]);
                          if Worlds.Values[tidWorldProp_Running + IntToStr(i)] = 'true'
                            then
                              begin
                                ImageIndex := (i mod ((planetimages.Count div 2 )- 1)) + j;
                                Data := pointer(true);
                              end
                            else
                              begin
                                ImageIndex := (planetimages.Count div 2 )- 1 + j;
        ...
        then
          begin
           ...
           URL :=
            ...
            //new code
            //worldlogon.wlog.Create(BackgroundHandlerViewer.BackgroundHandlerView);
            //worldlogon.wlog.Parent:= BackgroundHandlerViewer.BackgroundHandlerView;
            //wlogon.twlogon.Parent:= BackgroundHandlerViewer.BackgroundHandlerView;
            //worldlogon.wlog.Top:=20;
            //worldlogon.wlog.Left:=350;
           // worldlogon.wlog.Notebook1.ActivePage:='logon';
            //worldlogon.wlog.Visible:= true;
            //worldlogon.wlog.UserName:=fUserName;
            //worldlogon.wlog.Password:=fPassword;
            //worldlogon.wlog.WorldName:=WorldList.Selected.Caption;
            //worldlogon.wlog.Area:=fWorlds.Values['Area'];
            //worldlogon.wlog.ISAddr:=fWorlds.Values[tidWorldProp_ISAddr + IntToStr(WorldList.Selected.Index)];
            //worldlogon.wlog.ISPort:=fWorlds.Values[tidWorldProp_ISPort + IntToStr(WorldList.Selected.Index)];
          ...
          try
            Ticker.Caption := GetFormattedLiteral('Literal407',
                                                   [
                                                     ...
                                                     fWorlds.Values[tidWorldProp_Nob + IntToStr(tmpItem.Index)],
                                                     fWorlds.Values[tidWorldProp_Reset + IntToStr(tmpItem.Index)]
                                                   ]
                                                 );
            if ((SelItem<>nil) and (fWorlds.Values[tidWorldProp_Rules + IntToStr(SelItem.Index)]<>''))
              then
                begin
                  ...
                  RulesTicker.Caption := fWorlds.Values[tidWorldProp_Rulest + IntToStr(tmpItem.Index)];//format(GetLiteral('Literal501'), [SelItem.Caption]);
