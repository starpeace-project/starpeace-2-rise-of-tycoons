...
interface
  ...
  uses
    ...
    Protocol, JukeBox, ChatHandler, Privacy, VCLUtils,music1,logs;
...
implementation
  ...
  function TOptionsHandler.HandleEvent( EventId : TEventId; var info ) : TEventHandlingResult;
    ...
    begin
      result := evnHandled;
      //logs.log('options','eventid here - ' + inttostr(eventid));
      case EventId of
        evnHandlerExposed :
          begin
            //logs.log('options','event 8');
            fMasterURLHandler.HandleEvent( evnAnswerConfigHolder, ConfigHolder );
            //logs.log('options','event 8 -2');
            if fPlayList = nil
              then fMasterURLHandler.HandleEvent( 90126789, fPlayList );//InitPlayList( fCachePath + tidFolderName_UserData + fClientView.getUserName + '\' + tidFileName_PlayList );
            if fIgnoreList = nil
              then HandleEvent( 90126788, PrivacyHandler );

            //logs.log('options','event 8 -2.1');
            ...
            try
              ...
              //none
              HandleEvent( 90126788, PrivacyHandler );
            end;
            //logs.log('options','event 8 -2.3');
            fControl.Playlist.Items.BeginUpdate;
            //logs.log('options','event 8 -2.4');
            try
        ...
        evnGetPlaylist :
          begin
            //logs.log('options','playlist');
            fcontrol.Timer1.Interval:=1000;
            PlayList := self;
            if fPlayList = nil
              then InitPlayList( fCachePath + tidFolderName_UserData + fClientView.getUserName + '\' + tidFileName_PlayList );
            //logs.log('options','playlist - ' + fCachePath + tidFolderName_UserData + fClientView.getUserName + '\' + tidFileName_PlayList);
          end;
        evnAnswerPrivacyHandler :
          begin
            //logs.log('options','privdata0');
            PrivacyHandler := self;
            if not fLoaded
              then
                begin
                  //logs.log('options','privdata0.1');
                  LoadPrivData;
                  fLoaded := true;
                  //logs.log('options','privdata0.2');
                end;
          end;
        evnKeyCommand :
          case KeyCommand of
  ...
  procedure TOptionsHandler.SelectNextFile;
    ...
    begin
      ...
      s1:=  fPlayList.Strings[fSelectedFile];
      //logs.log('options',' s1 - ' + s1);
      music1.Form2.mplay.FileName := s1;
      if s1 <> '' then
        begin
      ...
      //if fPlayList.count = 0 then fcontrol.Timer1.Interval := 10000 else fcontrol.Timer1.Interval:=1000;
  ...
  procedure TOptionsHandler.JumpToFile( index : integer );
    begin
      ...
      if (fControl.Playlist <> nil) and (fSelectedFile >= 0)
         then fControl.Playlist.Items[fSelectedFile].StateIndex := -1;
      fSelectedFile := index;
      music1.Form2.mplay.FileName := fPlayList.Strings[index];
      music1.Form2.mplay.Open;
      music1.Form2.mplay.Play;
      if (fControl.Playlist <> nil) and (fSelectedFile >= 0)
         then fControl.Playlist.Items[fSelectedFile].StateIndex := 0;
