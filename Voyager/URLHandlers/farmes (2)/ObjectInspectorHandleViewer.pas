...
uses
  ...
  StdCtrls, VisualClassesHandler,variants, InternationalizerComponent, VCLUtils;
...
implementation
  ...
  function TObjectInspectorContainer.GetCacheConnection : IRDOConnectionInit;
    ...
    begin
      logs.log('objectview', 'getcacheconnection - ' + inttostr(fclientview.getCachePort));
  ...
  function TObjectInspectorContainer.GetMSConnection : IRDOConnectionInit;
    begin
      logs.Log('objectview',inttostr(fmsconn.port));
      if (fmsconn.port <> fClientView.getDALockPort) then
        begin
           fmsconn.Disconnect;
           fmsconn :=nil;
        end;
  ...
  function TObjectInspectorContainer.GetProperties(Names : TStringList) : TStringList;
    ...
    begin
      ...
      try
        //Lock;
        logs.log('objectview', 'start navigator');
        StartNavigator;
        try
    ...
    begin
      ...
      try
        ...
        if not VarIsEmpty(srvProxy)
          then
            try
              logs.log('objectview', 'createcacheobjectproy');
              Connection := GetCacheConnection;
  ...
  function TObjectInspectorContainer.GetCacheObjectProxy : OleVariant;
    ...
    begin
      try
        currObj := fObjectId;
        logs.log('objectview', 'getcacheobjectproy');
        //Lock;
        try
  ...
  procedure TObjectInspectorHandlerViewer.ChangeObject(ClientView : IClientView; xPos, yPos, ClassId, ObjectId : integer);
    ...
    begin
      try
        ...
        logs.log('objectview', 'add control tabs');
        AddControlTabs;
        logs.log('objectview', 'refresh');
        Refresh;
        logs.log('objectview', 'showimage');
        //Tabs.CurrentTab := 0;
  ...
  function TObjectInspectorHandlerViewer.undofconn;
    begin
        fPropSheetContInit  := nil;
        fPropSheetContainer := nil;
       result:='a';
    end;
