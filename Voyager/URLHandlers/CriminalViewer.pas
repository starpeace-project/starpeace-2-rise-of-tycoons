...
uses
  ...
  VoyagerInterfaces, VoyagerServerInterfaces, messagebox,
...
type
  TCriminalView = class(TForm)
    ...
    procedure SalaryBarMoveBar(Sender: TObject);
...
{$R *.DFM}
  ...
  procedure TCriminalView.SetProperties( Prop : TStringList );
    ...
    begin
      try
        ...
        teams.Text := fIllSystem.RDOGetTeams( fLeader.Values['Name'] );
        teamlist.Clear;
        teamlist.Items.Assign(teams);
        for i:=0 to pred(teams.count) do
          begin
            if  fTeam.Values['Name']=teamlist.Items.Strings[i] then teamlist.ItemIndex :=i;
          end;
        teams.free;  
      except
      end;
  ...
  var
    s,r:string;
    begin
    s := 'Fire this crininal';
  r := 'Are you sure you want to fire this criminal?';
  if ShowMsgBoxYN(r, s, 1, true, true)=mrOk then
    begin
      //logs.Log('change team','disband the team is ok');
  ...
  procedure TCriminalView.SalaryBarChange(Sender: TObject);
    ...
    e:integer;
    begin
      // change salary SalaryValue.Caption := IntToStr(SalaryBar.Value) + '%';
     e:= fIllSystem.RDOChangeCriminalSalary(fLeader.Values['Name'], fProperties.Values['Name'], SalaryBar.Value);
     SalaryValue.Caption := IntToStr(SalaryBar.Value) + '% ($' + IntToStr( round(StrToFloat(fProperties.Values['Salary']) *(SalaryBar.Value / strtoint(fProperties.Values['SalaryPerc'])))) + '/h)';
    end;
...
procedure TCriminalView.SalaryBarMoveBar(Sender: TObject);
begin
 // SalaryValue.Caption := IntToStr(SalaryBar.Value) + '% ($' + IntToStr( round(StrToFloat(fProperties.Values['Salary']) *(SalaryBar.Value / strtoint(fProperties.Values['SalaryPerc'])))) + '/h)';
end;

procedure TCriminalView.btMoveClick(Sender: TObject);
var
  s,r:string;
  x:integer;
begin
  s := 'Move criminal to team ' + teamlist.Items.Strings[teamlist.ItemIndex];
  r := 'Are you sure you want to move this criminal?';
  if ShowMsgBoxYN(r, s, 1, true, true)=mrOk then
    begin
      //logs.Log('change team','disband the team is ok');
      x:=fIllSystem.RDOChangeTeam(fLeader.Values['Name'], fTeam.Values['Name'], teamlist.Items.Strings[teamlist.ItemIndex], fProperties.Values['Name']);
      fOnTeamModified;
    end;
end;
