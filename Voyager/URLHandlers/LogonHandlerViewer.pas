...
uses
  ...
  InternationalizerComponent, Config, jpeg,variants,
  OleCtrls, SHDocVw,   {MapIsoView,}
  SHDocVw_TLB;
...
type
  TLogonHandlerView = class(TVisualControl)
    ...
    Panel1: TPanel;
    backtologin: TFramedButton;
    cnaccount: TFramedButton;
    Edit1: TEdit;
    Edit2: TEdit;
    Edit3: TEdit;
    Label2: TLabel;
    ...
    procedure edLogonPasswordKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure backtologinClick(Sender: TObject);
    procedure cnaccountClick(Sender: TObject);
  private
...
{$R *.DFM}
  ...
  uses
    ...
    RDORootServer, logs, LogFile, WinSockRDOConnection, RDOServer, RDOObjectProxy, Protocol, ShellAPI,
  ...
  procedure TLogonHandlerView.Start;
    ...
    begin
      ...
      DSAddr := 'starpeacelive.com';  //fConfigHolder.ReadString( true, '', 'DSAddr', 'dir.legacyonline.net' );
      DSPort := 1111; //fConfigHolder.ReadInteger( true, '', 'DSPort', 1111 );
      ...
      if (DefSite = '') or (system.pos('64.69.183.3', DefSite) <> 0)
        then
          begin
            DefSite := 'http://www.starpeacelive.com/five/';
                    ...
                    begin
                      try
                        ...
                        DSCode := DSProxy.RDOLogonUser( MasterUser, Password );
                               if (DSCode = DIR_NOERROR) then   advert := 2 else advert:= 3;
                        //logs.Log('logon','login dscode - ' + inttostr(dscode));
                        case DSCode of
  ...
  procedure TLogonHandlerView.syncGetSignIn( const parms : array of const );
    ...
    begin
      ...
      ConnectingWin.Hide;
       {if (DSCode = DIR_NOERROR)
        then   MapIsoView.advert := 2 else MapIsoView.advert:= 3;  }
...
{      ConnectingWin.Left := Screen.Width - ConnectingWin.Width;
     ...
     // ShellExecute(Handle,'open', 'http://www.starpeacelive.com/a1/newaccount.asp', nil, nil,  SW_SHOW);
     // label4.Visible := true;
      panel1.Visible:=true;
      panel1.BringToFront; 

      //newaccountbrowser.Navigate('http://sp1.starpeace2.com/Five/0/Visual/Voyager/NewLogon/createDemoAccount.asp');
    end;
...
procedure TLogonHandlerView.backtologinClick(Sender: TObject);
begin
panel1.Visible:=false;
end;

procedure TLogonHandlerView.cnaccountClick(Sender: TObject);
    var
      DSCnx     : IRDOConnectionInit;
      DSProxy   : OleVariant;
      session,i1   : integer;
begin
    cnaccount.Enabled:= false;
    Label10.Caption:= '........ connecting, please wait';
    i1:= 0;
// create link to ds, set info, get back message, set alert depending on result
              DSCnx        := TWinSockRDOConnection.Create( 'Directory Server' );
              DSCnx.Server := fDSAddr;
              DSCnx.Port   := 2222;
              DSProxy      := TRDOObjectProxy.Create as IDispatch;
                      ...
                      if session <> 0
                        then
                          begin
                            try
                              DSProxy.BindTo( session );
                              i1 := DSProxy.RDOcaAccount( edit1.text,edit2.text,edit3.text,edit4.text,combobox1.text , strtoint(combobox2.text),strtoint(combobox3.text),strtoint(combobox4.text) );
          ...
          case i1 of
          0: label10.Caption := '..Could not connect to server';
          1: label10.Caption := 'Account Created';
          2: label10.Caption := 'Username Already Taken';
          3: label10.Caption := 'Please fill in all fields';
          4: label10.Caption := 'An Unknown Error Occured';
          end;
         cnaccount.Enabled:= true;
end;
