...
uses
  ...
  VoyagerInterfaces, VoyagerServerInterfaces, CrimeProtocol, variants, logs,FiveControl,MapIsoView,
  InternationalizerComponent,WinSockRDOConnection, RDOInterfaces, RDOObjectProxy, Protocol,Messagebox;
...
type
  TCrimeMainView = class(TVisualControl)
    ...
    procedure btTeamHistoryClick(Sender: TObject);
    procedure btGotoHQClick(Sender: TObject);
    procedure btSetHQClick(Sender: TObject);
    procedure btMissionClick(Sender: TObject);
    procedure btDisbandTeamClick(Sender: TObject);
...
{$R *.DFM}
  ...
  function CreateIllSystemProxy : olevariant;
        ...
        if IBCnx.Connect( 1000 )
          then
            begin
              ...
              logs.Log('crimehandler','sys prox ');
            end
          else
            begin
            result := null;
    ...
    begin
      fIllSystem := CreateIllSystemProxy;//aIllSystem;
  ...
  procedure TCrimeMainView.threadedGetPlayerInfo( const parms : array of const );
    ...
    begin
      try
        ...
        logs.Log('crimeviewer', 'leader name = ' + leadername + ' username = ' + fClientView.getUserName);
        if leadername = '1' then leadername:='';
        if LeaderName <> ''
          then
            begin
  ...
  procedure TCrimeMainView.syncGetPlayerInfo( const parms : array of const );
    ...
    begin
      try
        ...
        lbmoney.Caption :=  fLeaderInfo.Values['Money'];
        lbrank.Caption := fLeaderInfo.Values['Expenses'];
  ...
  function CreateIllSystemProxy : olevariant;
    ...
    begin
   Fork( threadedGetPlayerInfo, priNormal, [self] );
    //IllSystem        := CreateIllSystemProxy;
    end;
...
procedure TCrimeMainView.CloseBtnClick(Sender: TObject);
begin
   fMasterURLHandler.HandleURL( '?frame_Id=CrimeHandler&frame_Close=yes' );
end;

function TCrimeMainView.CreateIllSystemProxy : olevariant;
      var
        IBCnx : IRDOConnectionInit;
      begin
        IBCnx        := TWinSockRDOConnection.Create('Illegal Business Component');
...
procedure TCrimeMainView.btTeamHistoryClick(Sender: TObject);
begin
{team history}
      HistoryDlg.Team := fCurrTeamInfo;
      HistoryDlg.htype :=2;
      HistoryDlg.Leader := fLeaderInfo;
...
procedure TCrimeMainView.btGotoHQClick(Sender: TObject);
var
  x,y,a,b,c: integer;
begin
{get co ords and goto}
  x:=strtoint(fCurrTeamInfo.Values['HeadquarterX']);
  y:=strtoint(fCurrTeamInfo.Values['Headquartery']);
  logs.Log('crimehq', 'goto hq' + fCurrTeamInfo.Values['Headquarterx'] + ' ' + fCurrTeamInfo.Values['Headquartery']);
  //fMapView
  //fthemap.fMapView.MoveTo(x,y);
  //fclientview.SetViewedArea(x,y,a,b,c);    MoveAndSelect
   fMasterURLHandler.HandleURL('?frame_Id=MapIsoView&frame_Action=MoveTo&x=' + fCurrTeamInfo.Values['HeadquarterX'] + '&y=' + fCurrTeamInfo.Values['Headquartery']);
end;

procedure TCrimeMainView.btSetHQClick(Sender: TObject);
var
  x,y:integer;
begin
{get co ords check it's you building and set}
//fclientview
  x:= MapIsoView.currx ;        //  .fCurrX
  y:= MapIsoView.curry ;
  logs.Log('crimehq','set hq to ' + inttostr(x) + ' - ' + inttostr(y));
  //temp bit, need to check the building is theres
  x:=fIllSystem.RDOAssignHeadquarter(fLeaderInfo.Values['Name'],cbTeams.text,x,y);
  logs.Log('crimehq', inttostr(x));
end;

procedure TCrimeMainView.btMissionClick(Sender: TObject);
begin
{gotomission viewer}
//MisionView
      MisionView.Team := fCurrTeamInfo;
      Misionview.Criminals := fCurrMemberInfo;
      MisionView.Leader := fLeaderInfo;
      //HistoryDlg.Criminal := fProperties;
      MisionView.IllSystem := fIllSystem;
      MisionView.MasterURLHandler := fMasterURLHandler;
      MisionView.ClientView := fClientView;
      misionview.MainView         := self;
      MisionView.ShowModal;
end;

procedure TCrimeMainView.btDisbandTeamClick(Sender: TObject);
var
  s,r:string;
  x:integer;
begin
  s := 'Disband ' + fCurrTeamInfo.Values['Name'];
  r := 'Are you sure you want to disbnd this team?';
  if ShowMsgBoxYN(r, s, 1, true, true)=mrOk then
    begin
      logs.Log('teamdestroy','disband the team is ok');
      x:=fIllSystem.RDODismissTeam(fLeaderInfo.Values['Name'], fCurrTeamInfo.Values['Name']);
      Fork( threadedGetPlayerInfo, priNormal, [self] );
    end;
end;

procedure TCrimeMainView.TabsTabChange(Sender: TObject);
begin
