...
interface
  ...
  uses
    ...
    RDOServer, ComCtrls, RDOInterfaces,StrUtils,variants, Classes, Collection, Matrix, SyncObjs, GMKernel, Privacy, VCLUtils,
    logs;
    ...
    TServerCnxHandler =
      class( TInterfacedObject, IMetaURLHandler, IURLHandler, IClientView )
        ...
        private
          ...
          function  NewCompany2( name, cluster : string; out ErrorCode : TErrorCode ) : string;
          ...
          function  GetCompanyName2(i1:integer) : string;
          function  GetCompanyOwnerRole(i4:integer)  : string;
          function  GetCompanyId2(i2:integer)   : integer;
          function  GetCompanyCluster(i3:integer) : string;
          function  buildkind :olevariant;
          function  buildlist(clusid,kindid:integer) :olevariant;
          function  currinfo  :olevariant;
          function  bankinfo  :olevariant;
          function  plinfo  :olevariant;
          function  resetaccount : integer;
          function  Advancelevel(i2:integer)   : integer;
          function  bankloanget(i2:widestring)   : integer;
          function  bankloanpay(i2:integer)   : integer;
        ...
        public
          ...
          fCompanyName : string;
          fCompanyId   : integer;
          fcompanylistid: integer;
          fcompanycluster: string;
          fMoney       : currency;
          fDate        : TDateTime;
          fSeason      : integer;
          fWorldURL    : string;
...
implementation
  ...
  uses
    ...
    ConnectingWindow, ObjectInspectorHandler, MessageBox,
    ...
    function SetCompany : TURLHandlingResult;
      ...
      begin
        try
          ...
          try
            fcompanylistid   :=  strtoint(URLParser.GetParmValue( URL, 'listid'));
          except
             fcompanylistid:=0;
          end;
          try
             fcompanycluster  := URLParser.GetParmValue( URL, 'cluster');
             if fcompanycluster='Dissident' then fcompanycluster:='Dissidents';
          except
             fcompanycluster :='none';
          end;
      ...
      case EventId of
        ...
        evnLogonStarted :
          begin
            if fISCnx <> nil
              then fISCnx.Disconnect;      
            //fMasterURLHandler.RegisterMetaHandler( TMetaObjectInpectorHandler.destroy );
            //fMasterURLHandler.RegisterMetaHandler( TMetaObjectInpectorHandler.Create );
            fExceptCount := 0;
            fISCnx := nil;
            fISProxy  := Unassigned;
            fPrivacyHandler := nil;
  ...
  function TServerCnxHandler.NewCompany2( name, cluster : string; out ErrorCode : TErrorCode ) : string;
    var
      TextualInfo : string;
            p1, p2 : integer;
      name1   : string;
    ...
    begin
      ...
      try
        ...
        if TextualInfo[1] = '['
          then
            begin
              p1 := mr_strutils.pos( ',', TextualInfo, 1 );
              if p1 > 0
                then
                  begin
                    name1 := copy( TextualInfo, 2, p1 - 2 );
                    p2 := mr_strutils.pos( ']', TextualInfo, p1 );
                    if p2 > 0
                      then
                        begin
                          ErrorCode := StrToInt(copy( TextualInfo, p1 + 1, p2 - p1 - 2 ));
                          result := name;
                        end
                      else result := ''
                  end
  ...
  //new
  function  TServerCnxHandler.getrank(rid:widestring)  :olevariant;
    begin
      result := fISProxy.getrank(rid);
    end;
  function  TServerCnxHandler.getranklist  :olevariant;
    begin
      result := fISProxy.getranklist;
    end;

  function  TServerCnxHandler.banksend(usr,amt:widestring)  :olevariant;
  begin
      //logs.log('wlogin','getcompanycount');
      result := fISProxy.banksend(usr,amt);
      //logs.log('wlogin','endgetcompanycount');
    end;

  function  TServerCnxHandler.gettowns  :olevariant;
   begin
      //logs.log('wlogin','getcompanycount');
      result := fISProxy.gettowns;
      //logs.log('wlogin','endgetcompanycount');
    end;

  function  TServerCnxHandler.capitol  :olevariant;
  begin
      //logs.log('wlogin','getcompanycount');
      result := fISProxy.capitol;
      //logs.log('wlogin','endgetcompanycount');
    end;

  function TServerCnxHandler.GetCompanyCount : integer;
    begin
      //logs.log('wlogin','getcompanycount');
      result := fISProxy.GetCompanyCount;
      //logs.log('wlogin','endgetcompanycount');
    end;

  function  TServerCnxHandler.GetCompanyName2(i1:integer) : string;
    begin
      result := fISProxy.GetCompanyName(i1);
    end;

  function  TServerCnxHandler.GetCompanyOwnerRole(i4:integer)  : string;
    begin
      result := fISProxy.GetCompanyOwnerRole(i4);
    end;

  function  TServerCnxHandler.GetCompanyId2(i2:integer)   : integer;
    begin
      result := fISProxy.GetCompanyId(i2);
    end;

  function  TServerCnxHandler.GetCompanyCluster(i3:integer) : string;
  begin
      result := fISProxy.GetCompanyCluster(i3);
    end;

  function  TServerCnxHandler.buildkind :olevariant;
  var
    x1: widestring;
  begin
      //logs.Log('buildmenu','startkind - 1.5');
      x1:= fcompanycluster ;
      result := fISProxy.buildkind(x1);
      //logs.Log('buildmenu','startkind - 1.6');
    end;

  function  TServerCnxHandler.buildlist(clusid,kindid:integer) :olevariant;
  var
    x1,x2: widestring;
  begin
      //logs.Log('buildmenu','startkind - 1.5');
      x1:= fCompanyName ;
      result := fISProxy.buildlist(x1,clusid,kindid); //set correctly, clusterid, kind id, companyname
      //logs.Log('buildmenu','startkind - 1.6');
    end;

  function  TServerCnxHandler.currinfo :olevariant;
  var
    x1,x2: widestring;
  begin
      //logs.Log('buildmenu','startkind - 1.5');
      x1:= fCompanyName ;
      result := fISProxy.currinfo; //set correctly, clusterid, kind id, companyname
      //logs.Log('buildmenu','startkind - 1.6');
    end;

  function  TServerCnxHandler.bankinfo  :olevariant;
   var
    x1,x2: widestring;
  begin
      //logs.Log('buildmenu','startkind - 1.5');
      x1:= fCompanyName ;
      result := fISProxy.bankinfo; //set correctly, clusterid, kind id, companyname
      //logs.Log('buildmenu','startkind - 1.6');
    end;

  function  TServerCnxHandler.plinfo  :olevariant;
   var
    x1,x2: widestring;
  begin
      //logs.Log('buildmenu','startkind - 1.5');
      x1:= fCompanyName ;
      result := fISProxy.plinfo; //set correctly, clusterid, kind id, companyname
      //logs.Log('buildmenu','startkind - 1.6');
    end;


  function  TServerCnxHandler.resetaccount : integer;
     begin
      result := fISProxy.resetaccount;
    end;

  function  TServerCnxHandler.Advancelevel(i2:integer)   : integer;
    begin
      result:= fISProxy.advancelevel(i2);
    end;

  function  TServerCnxHandler.bankloanget(i2:widestring)   : integer;
    begin
      result:= fISProxy.bankloan(i2);
    end;

  function  TServerCnxHandler.bankloanpay(i2:integer)   : integer;
  begin
      result:= fISProxy.bankloanpay(i2);
    end;
                                                        ...
                                                        then
                                                          begin
                                                            if (AccountStatus = ACCOUNT_Unexisting) or (fISProxy.GetCompanyCount = 0)
                                                              ...
                                                              //fWorldURL + ResultURL +
                                                              {'?frame_Id=LogonView&'+       {'frame_Target=' + Target + '&' +}
                                                              {'frame_Class=HTMLView&'+ }           //worldlogonView
                                                                  companycount := fISProxy.GetCompanyCount;
                                                                  //logs.log('wlogin','servercnxc count ' + inttostr(companycount));
                                                            SuccessURL := '?frame_Action=Show&frame_Id=wlogonView&frame_Class=worldlogonView&'+
                                                              'frame_Align=client&' + 'ResultType=' + ResultType + '&Logon=FALSE&' +
                                                              //'frame_NoBorder=True&frame_NoScrollBars=true&' +
                                                              'ClientViewId=' + IntToStr(fClientViewId) + '&' +
                                                              'WorldName=' + fWorldName + '&' +
                                                              'UserName=' + fUserName + '&' +
                                                              ...
                                                              'ISAddr=' + fISAddr + '&' +
                                                              'ISPort=' + IntToStr(fISPort) + '&frame_Visibility=VISIBLE&frame_Close=false';
                                                              //logs.log('wlogin','suscessurl - ' + SuccessURL);
                                                            ShowError( errNone, SuccessURL, Target );
                                                            //logs.log('wlogin','url handled ' );
                                                            //ShowError( errNone, '', Target );
                                                            //new login type:       wlogon.twlogon
                                                           { logs.log('wlogin','start1');
                                                            wlogon.twlogon.Top:=20;
                                                            wlogon.twlogon.Left:=345;
                                                            //worldlogon.wlog.Notebook1.ActivePage:='logon';
                                                            //worldlogon.wlog.Notebook1.PageIndex:=0;
                                                            //m1:= worldlogon.wlog.setpage(0);
                                                            logs.log('wlogin','start2');
                                                            wlogon.twlogon.Visible:= true;
                                                            wlogon.twlogon.BringToFront;
                                                            logs.log('wlogin','start3');
                                                            worldlogon.wlog.UserName:=fUserName;
                                                            worldlogon.wlog.Password:=fPassword;
                                                            worldlogon.wlog.WorldName:=fWorldName;
                                                            worldlogon.wlog.clientview :=IntToStr(fClientViewId);
                                                            worldlogon.wlog.ResultType := ResultType;
                                                            //worldlogon.wlog.Area:=fWorlds.Values['Area'];
                                                            worldlogon.wlog.ISAddr:=fISAddr;
                                                            worldlogon.wlog.ISPort:=IntToStr(fISPort);
                                                            worldlogon.wlog.DSAddr:=fDSAddr;
                                                            worldlogon.wlog.DSPort:=IntToStr(fDSPort);
                                                            //worldlogon.wlog.fISProxy :=fISProxy;
                                                           // m1:= worldlogon.wlog.login;    
 logs.log('wlogin','start');
case AnsiIndexText(ResultType,['NORMAL','NEWACCOUNT','ERROR']) of
  0:
    begin
      logs.log('wlogin','normal');
      wlogon.twlogon.companylist.Items.Clear;
      wlogon.twlogon.companylist.ViewStyle := vsIcon;
      logs.log('wlogin','clear');
      companycount := fISProxy.GetCompanyCount;
      logs.log('wlogin','c count ' + inttostr(companycount));
      for i  := 0 to companycount - 1 do
        with wlogon.twlogon.companylist.Items.add do
          begin
            logs.log('wlogin','cadd');
            caption := fISProxy.GetCompanyName(i);
            case AnsiIndexText(fISProxy.GetCompanyCluster(i),['pgi','dissidents','mariko','moab','magna']) of
               0:  ImageIndex:=0;
               1:  ImageIndex:=1;
               2:  ImageIndex:=2;
               3:  ImageIndex:=3;
      ...
      wlogon.twlogon.Panel2.BringToFront;
      //worldlogon.wlog.Notebook1.ActivePage:='sc';
    end;
      ...
      if Error = errNone
        then
          begin
            ...
            fMasterURLHandler.HandleEvent( evnLogonCompleted, Error );
            //logs.log('wlogin','suscessurl2 - ' + URL);
          end
        else
          if not Ignore
