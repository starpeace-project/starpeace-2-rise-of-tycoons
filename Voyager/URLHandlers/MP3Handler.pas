...
implementation
  ...
  uses
    URLParser, SysUtils, Events, Forms, SoundLib, logs;
  ...
  function TMP3Handler.HandleURL( URL : TURL ) : TURLHandlingResult;
    ...
    function Play( URL : string ) : TURLHandlingResult;
      ...
      begin
        ...
        logs.Log('music','media info');
        if (MediaId = '') and (MediaURL = '') and (fLastPlay<>'')
          then
            begin
              logs.Log('music','last play');
              MediaId := fLastPlay;
              fLastPlay := '';
            end;
