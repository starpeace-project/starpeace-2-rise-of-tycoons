     ...
     fWSISCnx      := TWinSockRDOConnection.Create('Interface Server');
       ...
       if canplay then
        ...
        //remove tycoon, and sho error
          ...
          label5.Visible:=true;
          //show label 5 error
          if i=-1 then  label5.Caption:='World is currently full';
          if i=-2 then  label5.Caption:='You are already playing in another world here';
          if i>0 then  label5.Caption:='You need ' + inttostr(i) + ' more nobility to play';
        end;
...
//join world on visitor visa
    fMasterURLHandler.HandleURL('?frame_Action=SetCompany&frame_Id=CnxHandler&Name=[VISITOR VISA]&Id=0&cluster=none&listid=0');
