...
uses
  ...
  StdCtrls, VisualClassesHandler,variants, InternationalizerComponent,
   VCLUtils,VisualClassManager,BuildClasses,MapTypes,classload,jpeg,IdHTTP,
  IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient,logs;
...
type
  ...
  public
    ...
    procedure ShowImage(const parms : array of const);
...
var
  ...
  lastusername,currentusername: string;
  xPos1, yPos1,flastclassid: integer;
...
implementation
    ...
    begin
      ...
      try
        ...
        try
          ...
          result.SaveToFile('x1.txt');
          l1:= result.Values['Creator'];
          if l1<> '' then lastusername:=l1 ;
          //ObjectInspectorHandlerViewer.showimage( fClientView.getWorldName, xPos1, yPos1 );
        finally
          StopNavigator;
          //Unlock;
        end;
  ...
  procedure TObjectInspectorHandlerViewer.ChangeObject(ClientView : IClientView; xPos, yPos, ClassId, ObjectId : integer);
    ...
    begin
      try
        ...
        Cursor := crHourGlass;
        fPropSheetContainer.ClearConnections;
  ...
  procedure TObjectInspectorHandlerViewer.ShowImage(const parms : array of const);
    var
      flags   : olevariant;
      useless : olevariant;
      limg:string;
      ...
      xPos, yPos : integer;
    begin
      WorldName := parms[0].vPchar;
      xPos := parms[1].vInteger;
      yPos := parms[2].vInteger;
      //  lastusername,currentusername: string;
  //xPos1, yPos1,flastclassid: integer;
      flags   := navNoHistory;
      useless := Null;
      IECompPanel.Visible := false;
      panel2.Visible := true;
      //logs.log('objectviewer','show image start');
      //fIEControl.Visible := false;
      if flastclassid <> fclassid then
        begin
           bsinfo := classload.getbuildinginfo(fClassId);
            limg:='cache\build\icon\' + bsinfo.ImagePath; //BuildingImages  build\icon
            image2.Picture.LoadFromFile(limg);
            flastclassid := fclassid;
        end;
      
      image2.Hint:= 'Located at ' + inttostr(xpos) + ',' + inttostr(ypos);
      noimage:= true;
      //try getting image 1 now
      if lastusername <> currentusername then
      begin
      //logs.log('objectviewer','image 2 done');
      limg:=fClientView.getWorldAbsURL + 'fivedata/userinfo/' + fClientView.getWorldName + '/' + lastusername + '/largephoto.jpg';
      //logs.log('objectviewer','lmg - ' + limg);
      try
        //logs.log('objectviewer','lmg - ' + limg);
        memStream := TMemoryStream.Create;
        //idhttp2:= TIdHTTP.Create;
        //logs.log('objectviewer','lmg - 2');
        idhttp1.Get(limg,memStream);
        //logs.log('objectviewer','lmg - 3 - ' + strStream);
        //idhttp2.Destroy;
        //logs.log('objectviewer','lmg - 4');
      except
      ...
      //logs.log('objectviewer','image 1 = ' + booltostr(noimage));
...
procedure TObjectInspectorHandlerViewer.Timer1Timer(Sender: TObject);
begin
 //timer for image changes
 fork(ShowImage,prinormal,[fClientView.getWorldName, xPos1, yPos1]);
end;
