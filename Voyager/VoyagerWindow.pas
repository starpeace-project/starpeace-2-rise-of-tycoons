...
uses
  ...
  InputSearchHandler, Config,topinfobar, music1,classload;
...
const
  ...
  tidFrameId_CriminalView  = 'CriminalView';
  tidFrameId_topinfobar  = 'topinfobar';
...
implementation
  ...
  uses
    ...
    VoiceHandler, LogonHandler, UniversalMapHandler,CrimeHandler, BackgroundHandler, ChartWindow, GMChatHandler,
    PlotterGrid, MessageBox, Literals, ClientMLS, OutputSearchHandlerViewer, InputSearchHandlerViewer,
    ChangeLog, Privacy, DisplayControl, CoolSB, FavView,logs,worldlogonhandler,buildHandler,searchviewhandler,tycoonhandler,tycoonshowhandler;
      ...
      case EventId of
        evnSetCompany :
          begin
            ...
            if logon
              then
                begin
                  ...
                  //  don't get maps anymore, use updater to do so
                  //SynchronizeWorld;
                  ...
                  //test build menu                                                                                                               &frame_Width=150
                  //fMasterURLHandler.HandleURL( '?frame_Action=Create&frame_Id=' + tidFrameId_BuildView + '&frame_Class=HTMLView&frame_Align=right&frame_NoBorder=True&frame_NoScrollBars=false&frame_Hidden=yes' );
                  ...
                  fMasterURLHandler.HandleURL( '?frame_Action=Create&frame_Id=CrimeHandler&frame_Class=CrimeHandler&frame_Align=right&frame_Width=375&frame_Hidden=yes' );
            ...
            fMasterURLHandler.HandleURL( '?frame_Action=Create&frame_Id=' + tidFrameId_BuildView + '&frame_Class=HTMLView&frame_Align=left&frame_Width=150&frame_Hidden=yes&frame_NoBorder=True&frame_NoScrollBars=false' );
            //fMasterURLHandler.HandleURL( '?frame_Id=' + tidFrameId_BuildView + '&frame_Action=Refresh' );
            fMasterURLHandler.HandleURL( '?frame_Action=Create&frame_Id=' + tidFrameId_DirectoryView + '&frame_Class=HTMLView&frame_Align=left&frame_Width=150&frame_Hidden=yes&frame_NoBorder=True&frame_NoScrollBars=false' );
            //fMasterURLHandler.HandleURL( '?frame_Id=' + tidFrameId_DirectoryView + '&frame_Action=Refresh' );
            // Toolbar
            fMasterURLHandler.HandleURL( '?frame_Action=Create&frame_Id=Toolbar&frame_Class=ToolbarView&frame_Align=bottom&frame_Height=100&frame_NoBorder=True&frame_NoScrollBars=True&frame_Hidden=no' );
  ...
  procedure TAppHandler.setMasterURLHandler( const URLHandler : IMasterURLHandler );
    begin
      ...
      topinfobar.tib.MasterURLHandler := fMasterURLHandler;
  ...
  procedure TVoyagerWin.FormCreate(Sender: TObject);
    ...
    begin
      ...
      fFrameSet.RegisterMetaHandler( TworldlogonHandler.Create );
      fFrameSet.RegisterMetaHandler( TbuildHandler.Create );
      fFrameSet.RegisterMetaHandler( Ttycoonhandler.Create );
      fFrameSet.RegisterMetaHandler( Tsearchviewhandler.Create );
      fFrameSet.RegisterMetaHandler( Ttycoonshowhandler.Create );
  ...
  {$endif}
      ...
      try
        ...
        //halt( 0 );
        //logs.Log('voice','create not working');
      end;
      ...
      fFrameSet.RegisterExclusion( tidFrameId_MailView, tidHandlerName_builder, true );//tidHandlerName_builder
      fFrameSet.RegisterExclusion( tidHandlerName_searchview, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_searchview, tidHandlerName_builder, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ChatList, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ChatList, tidHandlerName_builder, true );
      fFrameSet.RegisterExclusion( tidHandlerName_MsgComposer, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_MsgComposer, tidHandlerName_builder, true );
      fFrameSet.RegisterExclusion( tidFrameId_MessageView, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidFrameId_MessageView, tidHandlerName_builder, true );
      fFrameSet.RegisterExclusion( tidFrameId_MailView, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ChatList, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidHandlerName_MsgComposer, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidFrameId_MessageView, tidHandlerName_searchview, true );
      ...
      fFrameSet.RegisterExclusion( tidHandlerName_searchview, tidFrameId_WebMain, true );
      fFrameSet.RegisterExclusion( tidFrameId_BuildView, tidFrameId_WebMain, true );
      fFrameSet.RegisterExclusion( tidHandlerName_builder, tidFrameId_WebMain, true );

      fFrameSet.RegisterExclusion( tidFrameId_TycoonOpt, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidFrameId_TycoonOpt, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidFrameId_TycoonOpt, tidHandlerName_builder, true );
      ...
      fFrameSet.RegisterExclusion( tidFrameId_TycoonOpt, tidHandlerName_Crime, true );

      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidHandlerName_builder, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidHandlerName_ChatList, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidFrameId_MailView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidFrameId_MessageView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidHandlerName_Chat, true );
      fFrameSet.RegisterExclusion( tidHandlerName_tycoon, tidHandlerName_Crime, true );

      fFrameSet.RegisterExclusion( tidFrameId_UniverseMap, tidFrameId_TycoonOpt, true );
      fFrameSet.RegisterExclusion( tidFrameId_UniverseMap, tidHandlerName_tycoon, true );
      fFrameSet.RegisterExclusion( tidFrameId_UniverseMap, tidFrameId_WebMain, true );
      fFrameSet.RegisterExclusion( tidFrameId_UniverseMap, tidHandlerName_Crime, true );

      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidHandlerName_searchview, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidFrameId_BuildView, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidHandlerName_builder, true );
      ...
      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidHandlerName_tycoon, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidFrameId_WebMain, true );
      fFrameSet.RegisterExclusion( tidHandlerName_ObjInspector, tidHandlerName_Crime, true );
      // VoyagerTrains.RegisterMetaData;  // rag
      //new    fFrameSet.RegisterExclusion( , , true );
      fFrameSet.RegisterExclusion( 'tycoonshow', tidFrameId_WebMain, true );


      classload.loaded1:= false;
    end;
  ...
  procedure TVoyagerWin.FormShow(Sender: TObject);
    ...
    begin
      ...
      try
        ...
        fScrollMousePos.x := -1;
        fScrollMousePos.y := -1;
        // try to set the info bar
        topinfobar.tib.Height:=20;
        topinfobar.tbshow:=20;
        topinfobar.tib.Panel3.Visible:= false;
        topinfobar.tib.Panel2.Visible:= false;
        topinfobar.tib.cclearcur:= true;
        topinfobar.tib.cfunc:= 0;
        topinfobar.tib.Marquee1.Caption:='getting information';
        topinfobar.tib.Marquee1.hint:='getting information';
        topinfobar.tib.Timer1.Interval:=60000;
        topinfobar.tib.Align:=altop;
        topinfobar.tib.Parent:=FrameSetContainer;     // Voyagerwin.MsgPanel
        topinfobar.datetime:= now;
        topinfobar.dsupdate:=0;
        //topinfobar.tib.MasterURLHandler := fMasterURLHandler;
        ...
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=wlogonView&frame_Class=worldlogonView&frame_Hidden=yes' );
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=builderView&frame_Class=builderView&frame_Hidden=yes' );
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=tycoonView&frame_Class=tycoonView&frame_Hidden=yes' );
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=searchView&frame_Class=searchView&frame_Hidden=yes' );
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=tycoonshow&frame_Class=tycoonshow&frame_Hidden=yes' );
        // Play the introduction theme
     // vista sound problem 1
        //logs.Log('music','start intro');
        music1.Form2.mplay.FileName := 'Cache/sound/intro.mp3';
        music1.Form2.mplay.Wait := true;
        music1.Form2.mplay.Open;
        music1.Form2.mplay.Play;
     ...
     //   fFrameSet.HandleURL( '?frame_Id=MP3Handler&frame_Class=MP3Handler&frame_Action=Play&MediaId=IntroSoundTrack&MediaURL=intro.mp3&Loop=yes' );
        ...
        if (MasterSiteURL = '') or (MasterSiteURL = '-')
          then MasterSiteURL := 'http://www.starpeacelive.com/five/';
        ...
        fKeyMapper.AddAccelerator( VK_F6, fVIRTKEY, keytoptoolbar, false );
        ...
        fFrameSet.HandleURL( '?frame_Action=Create&frame_Id=CrimeHandler&frame_Class=CrimeHandler&frame_Align=left&frame_Width=400&frame_Hidden=yes' );
