...
uses
  ...
  MovieStudios in '..\..\StdBlocks\MovieStudios.pas' {$ifdef oldbuild},
  MagnaMarket in '..\..\StdBlocks\MagnaMarket.pas'{$endif};
...
procedure RegisterOffices;
  begin
    ...
    with TMetaFacility.Create( tidFacility_moff1, 'Magna Office', vidFacility_moff1, TFacility ) do
      begin
        XSize := 3;
        YSize := 3;
        Level := 120;
        ...
        EvlStages.Insert( TEvlStage.Create( tidFacilityStage_Construction, 'Construction', 'Requires construction materials to advance to the next stage.', TMetaBlock(TheClassStorage.ClassById[tidClassFamily_Blocks, tidBlock_moff1Constr])));
        EvlStages.Insert( TEvlStage.Create( tidFacilityStage_Complete, 'Building Completed', 'building is ready', TMetaBlock(TheClassStorage.ClassById[tidClassFamily_Blocks, tidBlock_moff1])));
        ClusterName := tidClusterName_Magna;
        FacilityKind := tidFacilityKind_MagnaBusinessFacilities;
        TechnologyKind := tidInventionKind_MagOffices;
...
procedure RegisterStores;
  begin
    ...
    with TMetaFacility.Create( tidFacility_MagnaMovieA, 'Magna Movie Theater', vidFacility_MagnaMovieA, TFacility ) do
      begin
        xSize := 4;
        ySize := 4;
        Level := 100;
        FacId := FID_MagnaMovie;
        EvlStages.Insert( TEvlStage.Create( 'Construction', 'Under Construction', 'Requires construction materials to advance to the next stage.', TMetaBlock(TheClassStorage.ClassById['Blocks', tidBlock_magnaMovieAConstr])));
        EvlStages.Insert( TEvlStage.Create( 'Complete', 'Completed', 'The Movie is ready to operate', TMetaBlock(TheClassStorage.ClassById['Blocks', tidBlock_magnaMovieA])));
        ClusterName := tidClusterName_Magna;
        FacilityKind := tidFacilityKind_MagnaMarkets;
        TechnologyKind := tidInventionKind_MagnaMovieTheaters;
        Register( tidClassFamily_Facilities );
      end;
  end;
