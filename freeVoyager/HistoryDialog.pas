...
type
  ...
  public
    ...
    property htype            : integer           write fhtype;
  private
...
implementation
  ...
  procedure THistoryDlg.threadedGetHistory( const parms : array of const );
    ...
    begin
      ...
      try
        if fhtype = 1 then  Info.Text := fIllSystem.RDORecoveryHistoryItem( '', '', fCriminal.Values['Name'] );
        if fhtype = 2 then  Info.Text := fIllSystem.RDORecoveryHistoryItem( fLeader.Values['Name'], fteam.Values['Name'] , '');
        if fhtype = 3 then  Info.Text := fIllSystem.RDORecoveryHistoryItem( fLeader.Values['Name'], '', '' );
