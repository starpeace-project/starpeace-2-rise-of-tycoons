  ...
  procedure TWorld.Simulate;
    ...
    procedure SimulateFacilities;
      ...
      begin
        ...
        assignfile(myfile,'testingfile.txt');
        rewrite(myfile);
        if update then desc := 'true' else desc := 'false';
        desc := desc + ' ' + floattostr(fdt);
        writeln(myfile,desc);
        if (fdt >= 1) and (dt < 100)
          then
            begin
              ...
              for i := 0 to pred(Facilities.Count) do
                with TFacility(Facilities[i]) do
                  try
                    fWorldLock.Enter;
                    try
                      desc :=  inttostr(VirtualTimeAbs) + 'virtual time ' + inttostr(i) + 'facility id ' + MetaFacility.Name + ' ' + MetaFacility.EvlStages.Items[1].ClassName + ' ' + MetaFacility.TypicalStage.Id + ' ' + MetaFacility.FacilityKind;
                      writeln(myfile,desc);

                      Profiler.ProcStarted( prfKind_Sim, prfId_Sim );
                      writeln(myfile,'profiler.procstarted');
                      Simulate;
                      writeln(myfile,'simulate');
                      Profiler.ProcEnded( prfKind_Sim, prfId_Sim );
                      writeln(myfile,'profiler.procended');
                      // >>> REMOVE!!!
                      //fNotify('Simulating...', round(100*i/Facilities.Count));
                      // >>>                       Logs.Log( tidLog_Junk,  );
                      if update
                    ...
                    on e : exception do
                      try
                        ...
                        writeln(myfile,TimeToStr(Now) + ' ' + desc + ' ' + e.Message + ' Error simulating facility ' + IntToStr(i));
                      except
                        writeln(myfile,TimeToStr(Now) + ' (big corruption) Error simulating facility ' + IntToStr(i));
