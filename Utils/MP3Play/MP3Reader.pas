...
implementation
  ...
  constructor TMP3Reader.Create(const aFileName : string; Loop : boolean);
    var
      ...
      DestSize : integer;
    begin
      //logs.Log('music','reader - create inherited');
      inherited Create;
      fLoop := Loop;
      fFIFO := TFIFO.Create;
      //logs.Log('music','fifo');
      fStream := CoAMMultiMediaStream.Create as IAMMultiMediaStream;
      //logs.Log('music','stream');
