        ...
        while found = 0 do
          begin
            ...
            repeat
               ...
               if CheckBox2.Checked = true then s1:=StringReplace(s1, '"', '',[rfReplaceAll, rfIgnoreCase]);
              ...
              // if CheckBox2.Checked = true then StringReplace(s2, '"', '',[rfReplaceAll, rfIgnoreCase]);
               //if CheckBox2.Checked = true then StringReplace(s3, '"', '',[rfReplaceAll, rfIgnoreCase]);
               names.Add(s2);
               names2.Add(s3);
               i:=i+1;
            until Eof(myfile);
...
while k < i do
  begin
  if checkbox1.Checked = false then writeln(myfile, names2.strings[k]) else writeln(myfile, names2.strings[k] + '<br>');
  k:=k+1;
  end;
closefile(myfile);
  ...
  begin
   ...
   //first check the folder exists, if not create it
    ...
    while k <= pos[tfiles-1] do
      ...
      readln(thefile, s1);
      s2 := names.strings[k] + '=';
      if CheckBox2.Checked = true then s2 := s2+ '"';
      s2 := s2 + s1;
      if CheckBox2.Checked = true then s2 := s2+ '"';
      writeln(myfile, s2);
      k:=k+1;
