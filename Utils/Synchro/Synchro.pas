...
implementation
  ...
  uses
    ...
    ShellAPI, FileCtrl, IniFiles, Masks, Literals,logs;
  ...
  procedure TSyncTask.Synchronize;
    ...
    function SynchronizeFiles( source, dest : string ) : TSyncErrorCode;
      begin
        //logs.Log('update', 'sfile ' + source + ' ' + dest);
    ...
    function SynchronizeFolders( source, dest : string ) : TSyncErrorCode;
      ...
      function GetIndexFile : TSyncErrorCode;
        var
          ...
          cancel : boolean;
        begin
          //logs.Log('update', 'indexfile 1 ' + source + ' ' + dest + ' - ' + syncmasterfile);
          if DownloadURLToFile( 'http://www.starpeacelive.com/five/client/' + syncMasterFile, dest + syncMasterFile, DownloadNotify ) = DWNL_NOERROR
            then result := SYNC_NOERROR
            else
              //logs.Log('update', 'indexfile 2 ');
      ...
      function GetFile( filename : string; Index : TFolderIndex; out ErrorInfo : string ) : TSyncErrorCode;
        ...
        begin
          ...
          if filename = 'borlndmm.dll' then freserve := 'bad';
          if filename = 'cabs.dll' then freserve := 'bad';
          if filename = 'fivelogs.dll' then freserve := 'bad';
          if filename = 'bin.cab' then freserve := 'bad';
         { case filename of
             : freserve:= 'bad';
             : freserve:= 'bad';
          end;  }
          if freserve <> 'bad' then
          begin
          repeat
            fFileSize := 0;
