...
interface
  ...
  uses                       
    ...
    Kernel, World, Windows, BackupInterfaces, MetaCrime, CrimeProtocol,logs;
  ...
  type
    ...
    CIllegalSystem = class of TIllegalSystem;
      ...
      class( TWorldExtension )
        ...
        public
          property Criminals    : TLockableCollection read fCriminals write fCriminals;
          property Leaders      : TLockableCollection read fLeaders write fLeaders;
  ...
  uses
    ...
    procedure TCriminal.CreateAttributes;
      ...
      begin
        ...
        logs.Log('createcriminal','attrlean');
        AddAttribute(AttrLean);
        logs.Log('createcriminal','addattribute');
        TrainItems := TCollection.Create(0, rkBelonguer);
        logs.Log('createcriminal','trainitems');
        ...
        for i := 0 to (TheClassStorage.ClassCount['Attribute'] - 1) do
          begin
            ...
            then
              TrainNumber := 6;
            logs.Log('createcriminal','att up to driving');
        ...
        logs.Log('createcriminal','salary');
    ...
    constructor TTeam.Create;
      begin
        logs.Log('teamcreation','start');
        inherited Create;
        logs.Log('teamcreation','inherited');
        fOwner := aOwner;
        fName := aName;
        logs.Log('teamcreation','owner/name');
        fRoles := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('teamcreation','roles');
        fHistoryItems := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('teamcreation','history');
        fHeadquarter.X := 0;
        fHeadquarter.Y := 0;
        fLawyersHours := 0;
        fBribing := 0;
        logs.Log('teamcreation','end');
      end;
    ...
    constructor TLeader.Create;
      begin
        inherited Create;
        logs.Log('crimeleader','inherited create');
        fTeams := TLockableCollection.Create(0, rkBelonguer);
        fHistoryItems := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('crimeleader','fteams and fhistoryitems');
        fOwner := aOwner;
        fPicture := aPicture;
        fName := aName;
        fRealName := aRealName;
        ...
        fExpenses := 0;
        fReputation := 0;
        logs.Log('crimeleader','setup values - owner,pic,name,money,expenses, rep');
        fCharges := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('crimeleader','fcharges');
        fNameList := TStringList.Create;
        logs.Log('crimeleader','fnamelist');
        fNLChange := 0;
        fWarrant := 0;
        fJailTime := 0;
        fTrialDay := 0;
        ...
        fLawyersHours := 0;
        fBribing := 0;
        logs.Log('crimeleader','rest of values');
      end;
    ...
    constructor TIllegalSystem.Create;
      begin
        inherited;
        {logs.Log('IB','illsystem.create');
        illSystem.fCriminals := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('IB','fcrim');
        illSystem.fLeaders := TLockableCollection.Create(0, rkBelonguer);
        logs.Log('IB','flead');
        illSystem.FirstMNameList := TStringList.Create;
        logs.Log('IB','firstname');
        illSystem.FirstFNameList := TStringList.Create;
        logs.Log('IB','firstnamef');
        illSystem.LastNameList := TStringList.Create;
        logs.Log('IB','lastname');
        illSystem.CrimListChange := 0;
        logs.Log('IB','illsystem.create done'); }
      end;

    destructor TIllegalSystem.Destroy;
      begin
        IllSystem.fCriminals.Free;
        IllSystem.fLeaders.Free;
        inherited;
      end;

    procedure TIllegalSystem.AddCriminal( Criminal : TCriminal );
      begin
        IllSystem.fCriminals.Insert ( Criminal );
      end;

    procedure TIllegalSystem.AddLeader( Leader : TLeader );
      begin
        IllSystem.fLeaders.Insert ( Leader );
      end;
    ...
    function TIllegalSystem.RDOGetCriminalNames(LeaderName : widestring): olevariant;
      ...
      begin
        try
          leader := IllSystem.FindLeaderInSystem(LeaderName);
    ...
    procedure TIllegalSystem.PrepareCriminalNames(LeaderName : widestring);
      ...
      begin
        ...
        l := 1;
        m := 0;
        logs.Log('crimeleader','preparecrimnam - setup');
        MetaCrimList := TStringList.Create;
        logs.Log('crimeleader','preparecrimnam - create list');
        for j := 0 to (TheClassStorage.ClassCount['Criminal'] - 1) do
          ...
          then
            begin
              ...
              disp := False;
              logs.Log('crimeleader','preparecrimnam - disp false');
              if already = False
              then
                begin
                  ...
                  logs.Log('crimeleader','preparecrimnam - check disp');
                  if disp = True
                  then
                    begin
                      m := m + 1;
        ...
        logs.Log('crimeleader','preparecrimnam - totalmetacrim');
        if TotMetaCrim < 8
        then
        ...
        logs.Log('crimeleader','preparecrimnam - maxcrimaval');
        ...
        logs.Log('crimeleader','preparecrimnam - end maxcrimeaval');
        if (CrimLevel - 1) > 0
        then
          begin
            m := 0;
            ...
            TotMetaCrim := MetaCrimList.Count;
            logs.Log('crimeleader','preparecrimnam - totalmetacrim list count');
            if TotMetaCrim < 4
            then
              begin
        ...
        logs.Log('crimeleader','preparecrimnam - big code here, most likly cause somewhere in this section');
        if (CrimLevel - 2) > 0
        then
          begin
            m := 0;
        ...
        then
          begin
            logs.Log('crimeleader','preparecrimnam - crimlevel - 3');
            m := 0;
            MetaCrimList.Clear;
            for j := 0 to (TheClassStorage.ClassCount['Criminal'] - 1) do
        ...
        then
          begin
          logs.Log('crimeleader','preparecrimnam - crimlevel - 4');
            m := 0;
            MetaCrimList.Clear;
            for j := 0 to (TheClassStorage.ClassCount['Criminal'] - 1) do
        ...
        CrimListChange := 0;
        leader.NLChange := 1;
        logs.Log('crimeleader','preparecrimnam - end');
      end;
    ...
    function TIllegalSystem.CriminalWrapper(Criminal : TCriminal): TStringList;
      ...
      begin
        ...
        if criminal.State = TCriminalState(4) then
        s:=' ' + inttostr(round(criminal.TrainingStage/criminal.TrainingClass.Time*100)) + '%'
        else
        s:='';
        List.Values['TrainingStage']  := IntToStr(Criminal.TrainingStage) + s;     //
    ...
    function TIllegalSystem.RDOGetMissionReport(LeaderName: widestring; TeamName : widestring) : olevariant;
      begin
        try
          Result := IllSystem.FindTeamInSystem(LeaderName, TeamName).Mission.Report.Text;
        except
          Result := CRIME_ERROR_Unknown;
        end;
      end;
    ...
    function TIllegalSystem.LeaderWrapper(Leader : TLeader): TStringList;
      var
        List : TStringList;
      begin
        List := TStringList.Create;         //leader.Expenses
    ...
    function TIllegalSystem.RDOCreateLeader(Name : widestring; RealName : widestring; Picture : widestring): olevariant;
      ...
      begin
        try
          logs.Log('crimeleader','start the creation of new leader');
          LeaderLean := TLeader.Create(IllSystem, Name, RealName, Picture);
          logs.Log('crimeleader','tleader created');
          ...
          logs.Log('crimeleader','tleader history created');
          IllSystem.AddLeader(LeaderLean);
          logs.Log('crimeleader','add leader to system');
          HandleCriminalsDisponibility;
          logs.Log('crimeleader','handle crimdispon');
          PrepareCriminalNames(Name);
          logs.Log('crimeleader','preparecrimnames');
          Result := CRIME_NOERROR;
        except
          Result := CRIME_ERROR_Unknown;
          logs.Log('crimeleadererror','error has occured during creation of leader');
        end;
      end;
    ...
    procedure TIllegalSystem.CreateCriminal(Name : widestring; MetaCriminal : TMetaCriminal);
      ...
      begin
        logs.Log('createcriminal','start - ' + name );
        CriminalLean := TCriminal.Create(IllSystem, Name, MetaCriminal);
        logs.Log('createcriminal','criminal lean');
        IllSystem.AddCriminal(CriminalLean);
        logs.Log('createcriminal','add criminal');
        IllSystem.Criminals.Lock;
        try
          for i := 0 to (IllSystem.Criminals.Count - 1) do
          ...
          logs.Log('createcriminal','criminal create atributes');
        finally
          IllSystem.Criminals.Unlock;
        end;
    ...
    function TIllegalSystem.RDOCreateTeam(LeaderName : widestring; Name : widestring) : olevariant;
      ...
      begin
        ...
        try
          Leader := FindLeaderinSystem(LeaderName);
          logs.Log('createteam','leader find -' + leader.RealName + ' ' + leader.Name);
          case IllSystem.GetLeaderLevel(leader.Reputation) of
            0 : MaxTeams := 1;
            1 : MaxTeams := 2;
            2 : MaxTeams := 4;
          ...
          logs.Log('createteam','max teams -' + inttostr(maxteams) + ' - ' + inttostr(Leader.Teams.Count));
          if Leader.Teams.Count <= MaxTeams
            then
              begin
                IllSystem.fLeaders.Lock;
                logs.Log('createteam','leaders lock' );
                try
                  i := 0;
                  while (i < IllSystem.fLeaders.Count) and (TLeader(IllSystem.fLeaders[i]).FindTeam( name ) = nil) do
                    inc( i );
                  logs.Log('createteam','what is  i - ' + inttostr(i));
                  if i >= IllSystem.fLeaders.Count
                    then
                      begin
                        logs.Log('createteam','start making the team' );
                        Team := TTeam.Create(FindLeaderinSystem(LeaderName), Name);
                        logs.Log('createteam','team creation' );
                        ...
                        logs.Log('createteam','team history' );
                        Leader.AddTeam(Team);
                        logs.Log('createteam','add team to leader' );
                        Leader.AddHistoryItem(THistoryItem.Create(nil, nil, leader, TMetaHistoryItem(TheClassStorage.ClassById['HistoryItem', 'Leader12: TeamCreated']), IllSystem.Time, 'Founded team ' + Name, '', ''));
                        logs.Log('createteam','add history' );
                        result := CRIME_NOERROR;
                      end
                    else result := CRIME_ERROR_WrongTeamName
                finally
                  IllSystem.fLeaders.Unlock;
                end;
              end
     ...
     procedure TIllegalSystem.CreateRole(Owner : TTeam; MetaRole : TMetaRole);
      ...
      function TIllegalSystem.RDOFindLeaderName(RealName : widestring) : olevariant;
        ...
        begin
          try
            logs.Log('crimfindleader','start leaders.lock');
            IllSystem.Leaders.Lock;
            logs.Log('crimfindleader','leaders.lock');
            try
              i := 0;
              RealName := uppercase(RealName);
              logs.Log('crimfindleader','real name = ' + realname);
              logs.Log('crimfindleader','Name 0 is : ' + TLeader(IllSystem.Leaders[0]).Name + ' - ' + uppercase(TLeader(IllSystem.Leaders[i]).RealName) + ' ' + inttostr(IllSystem.Leaders.Count));
              while (i < IllSystem.Leaders.Count) and (uppercase(TLeader(IllSystem.Leaders[i]).RealName) <> RealName) do
                begin
                //logs.Log('crimfindleader','i = ' + inttostr(i) + 'leadername = ' + TLeader(IllSystem.Leaders[i]).RealName);
                i:=i + 1;
                end;
              logs.Log('crimfindleader',inttostr(i));
              if i < IllSystem.Leaders.Count
                then result := TLeader(IllSystem.Leaders[i]).Name
                else result := '';
              logs.Log('crimfindleader','Name found as : ' + TLeader(IllSystem.Leaders[i]).Name);
            finally
              IllSystem.Leaders.Unlock;
            end;
          except
            Result := CRIME_ERROR_Unknown;
            //logs.Log('crimfindleader','error : ' + result + ' ' + realname);
          end;
        end;
      ...
      function TIllegalSystem.RDOFindLeader(LeaderName : widestring) : olevariant;
        ...
        begin
          try
            ...
            if Leader <> nil
            then
              Result := IllSystem.LeaderWrapper(Leader).Text
            else
              Result := '';
          except
            Result := CRIME_ERROR_Unknown;
      ...
      function TIllegalSystem.RDOFindTeam(LeaderName : widestring; TeamName : widestring) : olevariant;
        ...
        begin
          try
            Team := IllSystem.FindTeamInSystem(LeaderName, TeamName);
            Result := IllSystem.TeamWrapper(Team).Text;
          except
            Result := CRIME_ERROR_WrongLeaderName;
          end;
        end;
      ...
      function TIllegalSystem.RDOHireCriminal(LeaderName : widestring; TeamName : widestring; CriminalName : widestring; SalaryPerc : integer) : olevariant;
        ...
        begin
          try
            IllSystem.FindTeamInSystem(LeaderName, TeamName).Hire(LeaderName, CriminalName, SalaryPerc);
            leader := IllSystem.FindLeaderinSystem(LeaderName);
            ...
            leader.NLChange := 1;
            crim := IllSystem.FindCriminalInSystem(CriminalName);
      ...
      function TIllegalSystem.RDODeassignMission(LeaderName : widestring; TeamName : widestring; Cancel : widestring; MissionName : widestring) : olevariant;
        ...
        begin
          try
            Leader := FindLeaderinSystem(LeaderName);
            team := FindTeamInSystem(LeaderName, TeamName);
            if team.Mission.Ready = true then cancel := 'yes' else cancel := 'no';
            if team.Mission.MetaMission.MissionType <> 'One-shot' then cancel := 'no';
            if Cancel = 'yes'
            then
              begin
      ...
      procedure TIllegalSystem.HandleCriminalsDisponibility;
        ...
        begin
          ...
          logs.Log('crimeleader','dispon - n-5');
          IllSystem.Leaders.Lock;
          logs.Log('crimeleader','dispon - leaders lock');
          try
            for i := 0 to (IllSystem.Leaders.Count - 1) do
             ...
             logs.Log('crimeleader','dispon - count leaders per level');
          finally
            IllSystem.Leaders.Unlock;
            logs.Log('crimeleader','dispon - leaders unlock');
          end;
          for n := 1 to 5 do
            begin
          ...
          logs.Log('crimeleader','dispon - criminals thing');
          for n := 1 to 5 do
            begin
              for j := 0 to (TheClassStorage.ClassCount['Criminal'] - 1) do
          ...
          logs.Log('crimeleader','dispon - class storage');
          for n := 1 to 5 do
            begin
          ...
          logs.Log('crimeleader','dispon - crimperlevel - error in here somewhere, most likly a non stopping loop');
          for n := 1 to 5 do
            begin
              logs.Log('crimeleader','dispon - crimperlevel - n ' + inttostr(n));
              for k := (CrimNumbPerLevel[n] + 1) to CrimNeededPerLevel[n] do
                begin
                  logs.Log('crimeleader','dispon - crimperlevel - k ' + inttostr(k));
                  l := -1;
                  m := 1;
                  ...
                  while GotMetaCrim = False do
                    begin
                      logs.Log('crimeleader','dispon - crimperlevel - gotmetacrim ' + booltostr(gotmetacrim));
                      l := l + 1;
                  ...
                  logs.Log('crimeleader','dispon - crimperlevel -  createcrim command next');
                  ...
                  logs.Log('crimeleader','dispon - crimperlevel -  createcrim end');
                end;
            end;
          logs.Log('crimeleader','dispon - crimnumlevel');
        end;
          ...
          case PeriodType of
            perDay :
              while PeriodCount > 0 do
                begin
                  try
                  logs.Log('ibtime','per day');
                  PropagateTheBeat;
                  dec( PeriodCount );
                  logs.Log('ibtime','end day');
                  except
                  logs.Log('ibtime','error in day');
                  end;
                end;
            perMonth :
              while PeriodCount > 0 do
                begin
                  try
                  logs.Log('ibtime','per month');
                  HandleCriminalsDisponibility;
                  CrimListChange := 1;
                  dec( PeriodCount );
                  logs.Log('ibtime','end month');
                  except
                  logs.Log('ibtime','error in month');
                  end;
                end;
         // end;
          end;  {$ENDIF}
      ...
      var
        i,j,k,ibleaders:integer;
        begin
          inherited;
          {//load ib system
          //load ib main stuff first
          fTime := Reader.ReadInteger( 'ibtime', 0 );
          ibleaders := Reader.ReadInteger( 'ibleaders', 0 );}
        end;
      ...
      var
        ...
        begin
          inherited;
        {  //store ib system   - to be completed
          //store ib main stuff first
          Writer.WriteInteger( 'ibtime', fTime );
          Writer.WriteInteger( 'ibleaders', fleaders.Count );
          //store leaders
          for i:=0 to pred(IllSystem.fLeaders.Count) do
            begin             
              //start leader saving
              leader:=  Tleader( IllSystem.fLeaders[i]);
              Writer.WriteString( 'leader-' + inttostr(i), leader.fName );
              Writer.WriteString( 'RName-' + inttostr(i), leader.fRealName );
              writer.WriteSingle('lmoney-' + inttostr(i), leader.fMoney);
              writer.WriteSingle('lexpense-' + inttostr(i), leader.fExpenses);
              writer.WriteInteger('lrep-' + inttostr(i), leader.fReputation);
              Writer.WriteString( 'lpic-' + inttostr(i), leader.fPicture );
              writer.WriteInteger('lnlc-' + inttostr(i), leader.fNLChange);
              writer.WriteSingle('lwarrant-' + inttostr(i), leader.fWarrant);
              writer.WriteInteger('ljt-' + inttostr(i), leader.fJailTime);
              writer.WriteInteger('ltd-' + inttostr(i), leader.fTrialDay);
              writer.WriteInteger('lted-' + inttostr(i), leader.fTrialEndDay);
              writer.WriteInteger('llh-' + inttostr(i), leader.fLawyersHours);
              writer.WriteSingle('lbribe-' + inttostr(i), leader.fBribing);
              writer.WriteInteger('lhistory-' + inttostr(i), leader.fHistoryItems.Count);
              for j:=0 to pred(leader.fHistoryItems.Count) do
                begin
                //store leaders history
                history:=thistoryItem(leader.fHistoryItems[j]);
                writer.WriteInteger('lh-' + inttostr(i) + '-date' + inttostr(j), history.fDate);
                Writer.WriteString( 'lh-' + inttostr(i) + '-event' + inttostr(j), history.fEvent );
                Writer.WriteString( 'lh-' + inttostr(i) + '-1param' + inttostr(j), history.fParam1 );
                Writer.WriteString( 'lh-' + inttostr(i) + '-2param' + inttostr(j), history.fParam2 );
                //fMetaHistoryItem  : TMetaHistoryItem;
                Writer.WriteString( 'lh-' + inttostr(i) + '-mhname' + inttostr(j), history.fMetaHistoryItem.Name );
                Writer.WriteString( 'lh-' + inttostr(i) + '-mhdesc' + inttostr(j), history.fMetaHistoryItem.Desc );
                Writer.WriteString( 'lh-' + inttostr(i) + '-mhtoo' + inttostr(j), history.fMetaHistoryItem.TypeOfOwner );
                end;
              writer.WriteInteger('lteams-' + inttostr(i), leader.fTeams.Count);
              for j:=0 to pred(leader.fTeams.Count) do
                begin
                 //store leaders teams
                end;
              writer.WriteInteger('lcharges-' + inttostr(i), leader.fCharges.Count);
              for j:=0 to pred(leader.fCharges.Count) do
                begin
                //store leaders charges
                end;
              writer.WriteInteger('lnamelist-' + inttostr(i), leader.fNameList.Count);
              for j:=0 to pred(leader.fNameList.Count) do
                begin
                //store namelist
                Writer.WriteString( 'lnl-' + inttostr(i) + '-' + inttostr(j), leader.fNameList.Strings[j] );
                end;
            end;
          //store criminals   fCriminals        : TLockableCollection;
          }
        end;
