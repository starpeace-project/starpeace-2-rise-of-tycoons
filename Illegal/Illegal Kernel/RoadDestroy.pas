...
interface
  ...
  uses
    SysUtils, IllegalKernel, Collection, ClassStorage, MetaCrime,logs;
...
implementation
  ...
  procedure TRoadDestroy.ProceedWithMission;
    begin
      logs.Log('roaddestroy','missiontime = ' + inttostr(missiontime));
      if ready = true then
      begin
      if MissionTime = 0
      then
  ...
  procedure TRoadDestroy.EndMission;
    var
      i : integer;
    begin
      logs.Log('roaddestroy','endmission');
      for i := 1 to 8 do
      ...
      logs.Log('roaddestroy','finnished');
    end;
  ...
  procedure TRoadDestroy.DriveToTheRoad(Diff: Single);
    var
      Driver : TCrimForTask;
    begin
      logs.Log('roaddestroy','drive to road');
      ...
      logs.Log('roaddestroy','end drive to road');
    end;
  ...
  procedure TRoadDestroy.PreparationForDestruction(Diff: Single);
    var
      Leader : TCrimForTask;
    begin
    logs.Log('roaddestroy','prep for destruction');
      ...
      logs.Log('roaddestroy','end prep for destruction');
    end;
  ...
  procedure TRoadDestroy.DestroyRoad(Diff: Single);
    var
      ...
      rep : string;
    begin
      logs.Log('roaddestroy','destroy road func');
      val := 0;
      if DestroyWith = 'Explosive'
      then
        begin
          ...
          for i := 1 to 8 do
            ...
            then
              Charges.Values['Offender1' + IntToStr(i)] := Roles.Name[i];
                ...
                crim := GetRightCriminalForTask('Gorilla', 'Brawling');
              ...
              then
                case IllSystem.UseSkill(val, Diff) of
                  1:
                    begin
                      Report.Values['DestroyRoad'] := name1  + ' made a very effective and quick job. None has any suspect';
                  ...
                  2:
                    begin
                      Report.Values['DestroyRoad'] := name1 + ' made a quick job. None has any suspect';
                      Experience.Values['Success4'] := name1;
                      Experience.Values['Skill4'] := 'Brawling';
                      Experience.Values['Deegre4'] := '1';
                      Experience.Values['Diff4'] := FloatToStr(Diff);
                  ...
                  3:
                    begin
                      NextDiff := 40;
                      Report.Values['DestroyRoad'] := name1 + ' made a very loud job. A pedestrian was souspicious and alerted the police';
                      PoliceAlerted := True;
                    end;
                  4:
                    begin
                      ...
                      //Report.Values['DestroyRoad'] := name1 + ' made a mess.  managed to get %s. A lot of people realized that something was wrong and alerted the police';
                      ...
                      Report.Values['DestroyRoad'] := name1 + ' made a mess and managed to get ' + rep + '. A lot of people realized that something was wrong and alerted the police';
                      PoliceAlerted := True;
                    end;
                end
              else
                case IllSystem.UseSkill(val, Diff) of
                  1:
                    begin
                      NextDiff := 30;
                      Report.Values['DestroyRoad'] := name1 + ' made a very effective and quick job. The team was able to leave before the police arrived';
                      Experience.Values['Success4'] := name1;
                      Experience.Values['Skill4'] := 'Brawling';
                      Experience.Values['Deegre4'] := '1';
                      Experience.Values['Diff4'] := FloatToStr(Diff);
                  ...
                  2:
                    begin
                      Report.Values['DestroyRoad'] := name1 + ' made a quick job. When they were about to leave, they realized that the police had arrived on the scene';
                      Experience.Values['Success4'] := name1;
                      Experience.Values['Skill4'] := 'Brawling';
                      Experience.Values['Deegre4'] := '1';
                      Experience.Values['Diff4'] := FloatToStr(Diff);
                  ...
                  3:
                    begin
                      Report.Values['DestroyRoad'] := name1 + ' made a very loud job and they took a long time. While they were still finishing the job, they realized that the police had arrived on the scene';
                      ShootingWithPolice(60);
                    end;
                  4:
                    begin
                      ...
                      Report.Values['DestroyRoad'] := name1 + ' made a mess. and managed to get ' + rep + '. While they were checking on him, they heard the police telling them to surrender';
                      ShootingWithPolice(70);
                    end;
                end;
        end;
      logs.Log('roaddestroy','end destroy road func');
    end;
  ...
  procedure TRoadDestroy.ControlPedestrians(Diff: Single);
    ...
    begin
      logs.Log('roaddestroy','control peds');
      ...
      logs.Log('roaddestroy','end control peds');
    end;
  ...
  procedure TRoadDestroy.PoliceArriving(Diff: Single);
    var
      Leader : TCrimForTask;
    begin
      logs.Log('roaddestroy','police arriving');
      Leader := GetRightCriminalForTask('Leader', 'Leadership');
      case IllSystem.UseSkill(Leader.SkillVal, Diff) of
        1:
          begin
      ...
      logs.Log('roaddestroy','end police arriving');
    end;
  ...
  procedure TRoadDestroy.ShootingWithPolice(Diff: Single);
    var
      ...
      healthHurt : integer;
    begin
      logs.Log('roaddestroy','shoot police');
      ...
      logs.Log('roaddestroy','end shoot with police - follow by endmission');
      EndMission;
    end;
  ...
  procedure TRoadDestroy.DriveAway(Diff: Single);
    var
      Driver : TCrimForTask;
    begin
      logs.Log('roaddestroy','drive away');
      Driver := GetRightCriminalForTask('Driver', 'Driving');
      case IllSystem.UseSkill(Driver.SkillVal, Diff) of
        1:
          begin
      ...
      logs.Log('roaddestroy','end drive away');
    end;
  ...
  procedure TRoadDestroy.CarChase(Diff: Single);
    var
      ...
      RolesToReass : TRolesInMission;
    begin
      logs.Log('roaddestroy','car chase');
      Driver := GetRightCriminalForTask('Driver', 'Driving');
      case IllSystem.UseSkill(Driver.SkillVal, Diff) of
        1:
          begin
      ...
      logs.Log('roaddestroy','end car chase');
    end;
  ...
  procedure TRoadDestroy.SackedIn(Diff: Single);
    var
      Leader : TCrimForTask;
    begin
      logs.Log('roaddestroy','sacked in');
      Leader := GetRightCriminalForTask('Leader', 'Leadership');
      case IllSystem.UseSkill(Leader.SkillVal, Diff) of
        1:
          begin
      ...
      logs.Log('roaddestroy','end sacked in');
    end;
  ...
  procedure TRoadDestroy.ArrestedWithInjuries;
    var
      ...
      healthHurt : integer;
    begin
      logs.Log('roaddestroy','arrested with inj');
      rep := '. ';
      j := 0;
      for i := 1 to 8 do
        if Roles.Name[i] <> ''
      ...
      logs.Log('roaddestroy','end arrested with inj followed by end mission');
      EndMission;
    end;
  ...
  procedure TRoadDestroy.Surrender;
    var
      ...
      RolesToReass : TRolesInMission;
    begin
      logs.Log('roaddestroy','surrender');
      for k := 1 to MaxCrimes do
        Charges.Values['PoliceDiff' + IntToStr(k)] := '0';
      for i := 1 to 8 do
        if Roles.Name[i] <> ''
      ...
      Roles := RolesToReass;
      logs.Log('roaddestroy','end surrender with end mission next');
      EndMission;
    end;
