...
type
  ...
  protected
    ...
    function Same(Entry: PHashedDirEntry; Hash: Cardinal; const Str: String):
        Boolean;
  public
    constructor Create(ReqSize: Cardinal = 521);
...
constructor TZipDirHashList.Create(ReqSize: Cardinal = 521);
begin
  inherited Create;
  fBlocks := 0;
  fLastBlock := nil;
  ...
  if ReqSize > 0 then
  begin
    if ReqSize < ChainsMin then
      ReqSize := ChainsMin
    else
    if ReqSize > ChainsMax then
      ReqSize := ChainsMax;
    SetLength(Chains, ReqSize);
    ZeroMemory(Chains, ReqSize * sizeof(PHashedDirEntry));
  end;
end;

destructor TZipDirHashList.Destroy;
begin
  Clear;
  inherited;
end;
