...
{          if OverwriteDest = OvrAlways then
      ...
      begin
        ...
        New(zde);
//{$IFDEF USE_HASH}
//        // -> Added by EGR: Add all filename to hash, for fast file find
//        FileIdx := DestZipMaster.ZipContents.Add(zde);
//        FNames.Add(UpperCase(zde^.FileName), FileIdx);
//{$ELSE}
...
begin
  ...
  FDirHashs := TZipDirHashList.Create(0);
//{$IFDEF USE_HASH}
//  // -> Added by EGR: Initial size > 0 required, else ValueOf() failes
//  FNames := TStringHash.Create(1);
//{$ENDIF}
...
//            // -> Added by EGR: Fast file find
//            tEntry := getEntry(FFSpecArgs.Strings[0]);
...
{$ENDIF}
//{$IFDEF USE_HASH}
//      // -> Added by EGR: Recreate with apropriate number of buckets
//      FNames.Free;
//      FNames := TStringHash.Create(EOC.TotalEntries);
//{$ENDIF}
...
{$ENDIF}
//{$IFDEF USE_HASH}
//          // -> Added by EGR: Add filenames to hash, for fast file find
//          FileIdx := ZipContents.Add(pzd);
//          FNames.Add(UpperCase(pzd^.FileName), FileIdx);
//{$ELSE}
          ZipContents.Add(pzd);
...
//{$IFDEF USE_HASH}
// -> Added by EGR: Ffast file find, if failed result is NIL
//function TZMWorker.GetEntry(const Name: String): pZipDirEntry;
//var
//  tIdx: Integer;
//begin
//  Result := nil;
//  tIdx := FNames.ValueOf(UpperCase(Name));
//  if tIdx >= 0 then
//    Result := GetDirEntry(tIdx);
//end;
//{$ENDIF}
