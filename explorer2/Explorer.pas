...
uses
  ...
  RDOInterfaces, RDOObjectProxy, ToolWin,StrUtils;
...
procedure TSQLExplorer.SpeedButton6Click(Sender: TObject);
var thekey,theletter:string;
  theItem   : TListItem;
  bool      : olevariant;
  i, lsPos  : integer;
  ListItem  : TListItem;
  ...
  sec, Kind : integer;
begin
{find user and display info}
thekey := 'root/users/';
theletter := LeftStr(edit2.Text,1);
NodeName := thekey + LowerCase(theletter) + '/' + edit2.Text;
label2.Caption := 'User - ' + NodeName;
try
      bool := ISProxy.RDOSetCurrentKey(NodeName);
      if not bool
        then
