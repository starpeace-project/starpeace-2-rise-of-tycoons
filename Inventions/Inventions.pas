...
interface
    ...
    TInvention =
      class( TMetaInstance )
        ...
        private
          ...
          fCatPos   : integer;
          fObsolete : boolean;
          fEffic2    : integer; //backup ones
          fQuality2  : integer;
          fEffic3    : integer; //backup ones
          fQuality3  : integer;
          fEfficiency2   : integer;
          fDesirability2 : integer;
          fCrimeRes2    : integer;
          fPollRes2     : integer;
          fPrivacy2     : integer;
          fBeauty2      : integer;
          fMaintenance2 : integer;
          fEfficiency3   : integer;
          fDesirability3 : integer;
          fCrimeRes3    : integer;
          fPollRes3     : integer;
          fPrivacy3     : integer;
          fBeauty3      : integer;
          fMaintenance3 : integer;
        private
        ...
        published
          ...
          property Effic2   : integer read fEffic2;//backup ones
          property Quality2 : integer read fQuality2;
          property Efficiency2   : integer read fEfficiency2;
          property Desirability2 : integer read fDesirability2;
          property CrimeRes2    : integer read fCrimeRes2;
          property PollRes2     : integer read fPollRes2;
          property Privacy2     : integer read fPrivacy2;
          property Beauty2      : integer read fBeauty2;
          property Maintenance2 : integer read fMaintenance2;
          property Effic3   : integer read fEffic3;//backup ones
          property Quality3 : integer read fQuality3;
          property Efficiency3   : integer read fEfficiency3;
          property Desirability3 : integer read fDesirability3;
          property CrimeRes3    : integer read fCrimeRes3;
          property PollRes3     : integer read fPollRes3;
          property Privacy3     : integer read fPrivacy3;
          property Beauty3      : integer read fBeauty3;
          property Maintenance3 : integer read fMaintenance3;
        public
  ...
  var inv1: array of tinv2;
 { var inv2: array of inv1;}

implementation

  uses
    SysUtils, ComObj, Kernel, Headquarters, ClassStorage, CompStringsParser, Protocol, SimHints,
    FacIds, Math, MathUtils, DelphiStreamUtils, TycoonLevels,logs;
  ...
  constructor TInvention.Load(xmlObj : OleVariant);
    ...
    begin
      ...
      fId    := theId;
      // General stuff
       //backup indy      if <0 then :=1 else :=0;
       try
        fQuality2 := GetProperty(Aux, tidInvAttr_Q);
        if fQuality2<0 then fQuality3:=1 else fQuality3:=0;
        except
        fQuality2 :=0;
        end;
      try
        fEffic2   := GetProperty(Aux, tidInvAttr_Effic);
        if fEffic2<0 then fEffic3:=1 else fEffic3:=0;
        except
        fEffic2   :=0;
        end;
      try
        //Logs.Log( 'research', GetProperty(Aux, tidInvAttr_Name) + ' ' + GetProperty(Aux, tidInvAttr_Q) + ' # ' + GetProperty(Aux, tidInvAttr_Effic) + ' - ' + inttostr(fQuality2) + ' # ' + inttostr(fEffic2));
        except
        end;
      //backup comm
      try
        fEfficiency2   := GetProperty(Aux, 'Efficiency');
        if fEfficiency2<0 then fEfficiency3:=1 else fEfficiency3:=0;
        except
        fEfficiency2 := 0;
        end;
      try
        fDesirability2 := GetProperty(Aux, 'Desirability');
        if fDesirability2<0 then fDesirability3:=1 else fDesirability3:=0;
        except
        fDesirability2 := 0;
        end;
      try
        //Logs.Log( 'research', GetProperty(Aux, tidInvAttr_Name) + ' ' + GetProperty(Aux, 'Efficiency') + ' # ' + GetProperty(Aux, 'Desirability')+ ' - ' + inttostr(fEfficiency2) + ' # ' + inttostr(fDesirability2));
        except
        end;
      //backupres
      try
        fCrimeRes2    := GetProperty( Aux, tidInvAttr_CrimeRes );
        if fCrimeRes2<0 then fCrimeRes3:=1 else fCrimeRes3:=0;
        except
        fCrimeRes2    :=0;
        end;
      try
        fPollRes2     := GetProperty( Aux, tidInvAttr_PollRes );
        if fPollRes2<0 then fPollRes3:=1 else fPollRes3:=0;
        except
        fPollRes2     :=0;
        end;
      try
        fPrivacy2     := GetProperty( Aux, tidInvAttr_Privacy );
        if fPrivacy2<0 then fPrivacy3:=1 else fPrivacy3:=0;
        except
        fPrivacy2     :=0;
        end;
     try
        fBeauty2      := GetProperty( Aux, tidInvAttr_Beauty );
        if fBeauty2<0 then fBeauty3:=1 else fBeauty3:=0;
        except
        fBeauty2      :=0;
        end;
     try
        fMaintenance2 := GetProperty( Aux, tidInvAttr_Maintenance );
        if fMaintenance2<0 then fMaintenance3:=1 else fMaintenance3:=0;
        except
        fMaintenance2 :=0;
        end;
      ...
      if StrToInt(TheGlobalConfigHandler.GetConfigParm('TornamentLength', '0')) = 0
        then try
          fNobility := GetProperty(Aux, tidInvAttr_Nobility);
          except
           fNobility:= 0;
           end
        else fNobility := 0;
      ...
      if fname = 'Banking Basics' then aux0 := Aux;
      {if VarSameValue(Aux, aux0) = true then Aux := Unassigned;}
      {if not VarIsEmpty(Aux)
        then OverrideTiers( Aux );}
      //set desc info for quick addition
      {Logs.Log( 'invdesc-'+fParent,'Inventions.' + theId +'.Name=' + fName );
      Logs.Log( 'invdesc-'+fParent,'Inventions.' + theId +'.Desc=' + fDesc );
      Logs.Log( 'invdesc-'+fParent,'Inventions.' + theId +'.Resp=' + fResp );

      Logs.Log( 'invdesc-ALLINONE','Inventions.' + theId +'.Name=' + fName );
      Logs.Log( 'invdesc-ALLINONE','Inventions.' + theId +'.Desc=' + fDesc );
      Logs.Log( 'invdesc-ALLINONE','Inventions.' + theId +'.Resp=' + fResp );
      Logs.Log( 'invdesc-THEPARENTS','Inventions.Category.' + fParent + '=' + fParent ); }
      //continue
      fName_MLS := TMultiString.Create;
      fDesc_MLS := TMultiString.Create;
      fResp_MLS := TMultiString.Create;
  ...
  function TInvention.GetProperties(Company : TObject; LangId : TLanguageId) : string;
    ...
    begin
      ...
      //other
      if fEffic2 <> 0
        then result := result + SimHints.GetHintText(mtidInvEff.Values[LangId], [FormatDelta(fEffic2)]) + LineBreak;
      if fQuality2 <> 0
        then result := result + SimHints.GetHintText(mtidInvQ.Values[LangId], [FormatDelta(fQuality2)]) + LineBreak;
      if fEfficiency2 <> 0
        then result := result + SimHints.GetHintText(mtidInvEff.Values[LangId], [FormatDelta(fEfficiency2)]) + LineBreak;
      if fDesirability2  <> 0
        then result := result + SimHints.GetHintText(mtidInvDesirability.Values[LangId], [FormatDelta(fDesirability2)]) + LineBreak;
      if fCrimeRes2 <> 0
        then result := result + SimHints.GetHintText(mtidInvCrimeRes.Values[LangId], [FormatDelta(fCrimeRes2)]) + LineBreak;
      if fPollRes2 <> 0
        then result := result + SimHints.GetHintText(mtidInvPollRes.Values[LangId], [FormatDelta(fPollRes2)]) + LineBreak;
      if fPrivacy2 <> 0
        then result := result + SimHints.GetHintText(mtidInvPrivacy.Values[LangId], [FormatDelta(fPrivacy2)]) + LineBreak;
      if fBeauty2  <> 0
        then result := result + SimHints.GetHintText(mtidInvBeauty.Values[LangId], [FormatDelta(fBeauty2)]) + LineBreak;
      if fMaintenance2 <> 0
        then result := result + SimHints.GetHintText(mtidInvMaintenance.Values[LangId], [FormatDelta(fMaintenance2)]) + LineBreak;
// Tier
  ...
  function TInvention.GetFullDesc( LangId : TLanguageId ) : string;
    var
      ...
      Inv : TInvention;
    begin
      result := Desc_MLS.Values[LangId] + LineBreak;
      if fEffic2 <> 0
        then result := result + SimHints.GetHintText(mtidInvEff.Values[LangId], [FormatDelta(fEffic2)]) + LineBreak;
      if fQuality2 <> 0
        then result := result + SimHints.GetHintText(mtidInvQ.Values[LangId], [FormatDelta(fQuality2)]) + LineBreak;
  ...
  function CreateInvention(InvObj, Appls : OleVariant) : TInvention;
    ...
    begin
      ...
      result := InvClass.Load(InvObj); {pack error here - on second invention i think}
      result.fNumId := LastInvention;
      try
      inv1[result.fNumId].fEffic2 := result.fEffic2 ;
      inv1[result.fNumId].fQuality2 := result.fQuality2 ;
      except
      end;
      try
      inv1[result.fNumId].fEfficiency2 := result.fEfficiency2 ;
      inv1[result.fNumId].fDesirability2 := result.fDesirability2;
      except
      end;
      try
      inv1[result.fNumId].fCrimeRes2 := result.fCrimeRes2 ;
      inv1[result.fNumId].fPollRes2 := result.fPollRes2 ;
      inv1[result.fNumId].fPrivacy2 := result.fPrivacy2 ;
      inv1[result.fNumId].fBeauty2 := result.fBeauty2 ;
      inv1[result.fNumId].fMaintenance2 := result.fMaintenance2 ;
      except
      end;
  ...
  procedure CreateInventions(path : string);
    ...
    begin
      ...
      try
        ...
        while found = 0 do
          begin
            //logs.Log('researchfiles',search.Name);
