...
interface
  ...
  uses
    Classes,Variants, Collection, RDOServer, RDOInterfaces, SyncObjs, DirectoryManager, DirectoryServerProtocol,SysUtils;
    ...
    TDirectorySession =
      class
        ...
        published
          ...
          function RDOcaAccount( username,password,email,realname,sex : widestring; dy,mth,yr : integer ) : olevariant;
          ...
          function RDOCanJoinNewWorld2( Alias,world : widestring ) : olevariant;
    ...
    TDirectoryServer =
      class
        ...
        private
          ...
          function CanJoinNewWorld2( Alias,world : widestring ) : boolean;
...
implementation
  ...
  uses
    Windows, WinSockRDOConnectionsServer, GenIdd, Logs, StrUtils, wininet, MainWindow,
  ...
  function TDirectorySession.RDOcaAccount( username,password,email,realname,sex : widestring; dy,mth,yr : integer ) : olevariant;
    var
      userkey: string;
    begin
      Lock;
      try
        keepAlive;
        if IsValidAlias( username ) then
          begin
             userkey   := GetUserPath(GetAliasId(username));
             if not RDOFullPathKeyExists( userkey )  then
              begin
                 if RDOCreateFullPathKey( userkey, true ) then
                  begin
                     RDOSetCurrentKey( userkey );
                     RDOWriteString( 'Password', Password );
                     RDOWriteString( 'Alias', username );
                     RDOWriteString( 'email', email );
                     RDOWriteString( 'name', realname );
                     RDOWriteString( 'sex', sex );
                     RDOWriteInteger( 'AccountStatus', 1 );
                     RDOWriteString( 'DOB', inttostr(dy) + '/' +inttostr(mth) + '/' +inttostr(yr));
                     RDOWriteDate( 'Created', Now );
                     RDOWriteDate( 'lastlog', Now );
                     result:=1;
                  end
                  else result:= 4;
              end
    ...
    begin
      try
        ...
        if fDirMng.SetCurrentKey(userkey)
          then
            begin
              dt2:= fDirMng.ReadDate('subdate');
              diff := dt2 - date;
              if diff < 0 then diff := 0;
              
              dt := date + days + diff;
              fdirmng.WriteString('accountstatus','6');
              result := fDirMng.WriteDate('subdate', dt);
            end
          else result := false;
      except
        result := false;
  ...
  function TDirectorySession.RDOCanJoinNewWorld( Alias : widestring ) : olevariant;
   ...
   function TDirectorySession.RDOCanJoinNewWorld2( Alias,world : widestring ) : olevariant;
    begin
      Lock;
      try
        KeepAlive;
        try
          result := fServer.CanJoinNewWorld2( Alias,world ) or DirectoryWin.cbJoinAllWorlds.Checked;
        except
          on e : Exception do
            begin
              fServer.LogThis( e.Message + ' @ TDirectorySession.RDOCanJoinNewWorld' );
...
//    MaxSerialUses = 1;
                    ...
                    if logpass <> ''
                      then
                        case acstatus of
                          ...
                          DIR_ACC_Newreguser : // registered paying players
                            if UpperCase(realpass) = UpperCase(Password)
                              then result := -1
                          ...
                          DIR_ACC_timecard :
                            ...
                            if UpperCase(realpass) = UpperCase(Password)
                              then
                                begin
                                //check the date of trial period to now
                                crtdate := session.RDOReadDate('subdate');
                                if crtdate - Now > 0 then
                                  result := 0
                                else
                                 result := -1;
                                end
                          ...
                          DIR_ACC_RegUser, DIR_ACC_TrialUser : // registered non paying players
                            begin
                            if UpperCase(realpass) = UpperCase(Password)
                              then
                                begin
                                crtdate := session.RDOReadDate('created');
                                if Now - crtdate <8 then
                                  result := 0
                                else
                                  result := DIR_SEGA_ERROR_trial;
                                end
                              else result := DIR_SEGA_ERROR_UserNotAuthorized;
                             end;
                          4 :
                    ...
                    if ((result = 0) or (result = -1))
                      then
                        begin
                          ...
                          session.RDOwriteDate('lastlogin', now);
                          // check newbie
                          if days > 15
  ...
  function TDirectoryServer.CanJoinNewWorld2( Alias,world : widestring ) : boolean;
    var
      session    : TDirectorySession;
      userkey    : string;
      NobPoints  : integer;
              ...
              if session.RDOFullPathKeyExists( userkey )
                then
                  begin
                   ...
                   alist.text:= session.RDOGetKeyNames;
                   j:=0;
                   logs.Log('canjoin','start ' + world);
                   for I := 0 to aList.Count - 1 do
                     begin
                       logs.Log('canjoin','value ' + alist.ValueFromIndex[i]);
                       if AnsiContainsText(alist.ValueFromIndex[i], 'merica') then      //alist.ValueFromIndex[i]
                          begin
                             if AnsiContainsText(alist.ValueFromIndex[i], world) then
                                begin
                                    //don't count
                                end
                              else
                   ...
                   // worldcount := session.RDOKeysCount;
                   // allowed    := NobilityToWorlds(NobPoints);
                   logs.Log('canjoin','j= ' + inttostr(j));
                   allowed := 1; //+ (NobPoints div 500);
                   if allowed > j then result     := true else result     :=false ;
                   logs.Log('canjoin','can join= ' + booltostr(result));
                   alist.Free;
                    //result     := true; //< NobPoints;
                  end
                else result := true;
  ...
  function GetProperties( data : string ) : TStringList;
    begin
      ...
      {StringReplace( data, '&', '#13#10', rfReplaceAll);
      StringReplace( data, '?', '#13#10', rfReplaceAll); }
              ...
              begin
                ...
                case accStat of
                  DIR_ACC_NoAuthUserA, DIR_ACC_NoAuthUserB :
                    expDate := Now;
                  DIR_ACC_timecard:
                    expDate := session.RDOReadDate('subdate');
                  DIR_ACC_TrialUser,DIR_ACC_BlockedUser,DIR_ACC_Newreguser:
                    begin
                    expDate := now;
                    if session.RDOValueExists('lastlogin') then
                      expDate := session.RDOReadDate('lastlogin')
                    else
                      session.RDOwriteDate('lastlogin',now);
                    end;
                  else
    ...
    begin
      case error of
        ...
        DIR_SEGA_ERROR_TimeOut :
          result := DIR_ERROR_Unknown;
        DIR_SEGA_ERROR_trial :
          result :=DIR_ERROR_TrialExpired;
        else
          result := error;
      end;
    end;
