  ...
  procedure TDirectorySession.RDOWriteInteger( Name : widestring; Value : integer );
    ...
    begin
      ...
      try
        ...
        logs.Log('RDOwrite',name + inttostr(value));
      finally
        UnLock;
      end;
    end;
                      ...
                      then
                        case acstatus of
                          DIR_ACC_BlockedUser : // blocked users
                           ...
                           DIR_ACC_RegUser, DIR_ACC_TrialUser,DIR_ACC_Newreguser, DIR_ACC_timecode : // registered paying players
                            if UpperCase(realpass) = UpperCase(Password)
                              then result := 0
        ...
        if session.RDOFullPathKeyExists( userkey )
          then
            begin
              ...
              userkey := GetUserPath( Alias ) + '/AccountInfo/freeWorlds';
              if session.RDOFullPathKeyExists( userkey )
                then
                  begin
                    session.SetCurrentKey( userkey );
