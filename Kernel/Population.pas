...
interface
  ...
  const   
    InmigrationTimeSlope    = 15*TimeUnits;    // Tendence to move in (newcomers only).
    EmigrationTimeSlope     = 20*TimeUnits;    // Tendence to leave town.
    OfsInmigrationTimeSlope = 15*TimeUnits;     // Tendence for offices to move in
    OfsEmigrationTimeSlope  = 25*TimeUnits;    // Tendence for offices to move out
    OfficesPerInhabitant    = 0.03;            // Offices generated by one inhabitant
    WorkersPerOffice        = 4;               // Workers per office
    ...
    SelfServRatio           = 0.4;             // Percent of services population is able to create by theirselves
    MaxMovers               = 500;              // Maximum number of people that can move to town in an hour
    MinimalPop              = 1000;
    ...
    OutWorkWeight         : array[TPeopleKind] of integer = (25, 20, 30);
    OutResidentialsWeight : array[TPeopleKind] of integer = (20, 20, 20);
    OutQOLWeight          : array[TPeopleKind] of integer = (80, 80, 80);
    OutUnemploymentWeight : array[TPeopleKind] of integer = (50, 50, 90);
  ...
  const
    EmigrationProb : array[TPeopleKind] of single  = (0.5, 0.38, 0.3);
    ...
    QOLvsUnemp     : array[TPeopleKind] of single  = (0.02, 0.04, 0.06);
...
implementation
  ...
  function TTownHall.Evaluate : TEvaluationResult;
          ...
          with TServiceInfo(fServiceInfo[i]) do
            begin
              ...
              for k := low(k) to high(k) do
                ...
                //x:= x + ' ' + floattostr(Population[k]) + '-' + floattostr(Kind.BuyProb[k]);
                end;
              Demand := Demand/OfferBoost;
              //logs.Log('pop1', inttostr(i) + ' ' + floattostr(demand) + x);
              Offer := Facility.Town.Parameters[Kind.TownParameterIds[tidTownParameter_ServiceSales]].Value;
              if Offer = 0 then Offer := 1;
              if Demand > 0
    ...
    begin
      ...
      KsalAvg           := realmin(1,(KsalAvg/ksalco));//KsalAvg/2.65;
  ...
  procedure TTownHall.RDOSetTaxValue( TaxId : integer; Value : widestring );
    var
      Tax : TTax;
    begin
      Logs.Log( 'tax', TimeToStr(Now) + ' Setting Tax value: ' + Facility.Town.Name + ', ' + IntToStr(TaxId) + ', ' + Value );
      try
  ...
  procedure TTownHall.RDOSetMinSalaryValue( PopKind, Value : integer );
    begin
      Logs.Log( 'wage', TimeToStr(Now) + ' Setting Min Wage: ' + Facility.Town.Name + ', ' + IntToStr(integer(PopKind)) + ', ' + IntToStr(Value) );
      try
      ...
      Logs.Log( 'wage',  'Setting Min Wage OK!');
    end;
