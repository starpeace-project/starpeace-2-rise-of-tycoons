...
interface
  ...
  uses
    ...
    Tasks, Tutorial, TycoonVotes, Favorites, CloneOptions,types, BackupObjects;
  ...
  type
    ...
    IRoadHandler =
      interface
        procedure NearestCircuitsToArea( CircuitId : TCircuitId; a,b,c,d:integer; var Circuits : TCollection ); //commerce fucked here when added
    ...
    TTown =
      class( TMoneyDealer )
        ...
        published
          ...
          property tournp   : integer       read ftournp   write ftournp;
        private
    ...
    TTycoon =
      class( TMoneyDealer )
        ...
        published
          ...
          property EffectProgress  : single read fEffectProgress write fEffectProgress;
          property EffectStrength  : single read fEffectStrength write fEffectStrength;
          property tournteam        : integer   read ftournteam       write ftournteam;
          property ttname           : string read fttname write fttname;
        protected
        ...
        public
          ...
          property Tton : boolean read ftton write ftton;
      end;
...
implementation
  ...
  function TFacility.ConnectTo( Facility : TFacility ) : string;
    begin
      try
        if not Facility.Deleted and (Facility.Company <> nil) and not Facility.Company.Deleted and (((tournton=true) and (facility.Company.Owner.tournteam = Company.Owner.tournteam))or(tournton = false))
          then
            begin
                    ...
                    try
                      for i := 0 to pred(Gates.Count) do
                        with TGateDesc(Gates[i]) do
                          begin                              //orggate.Block.Facility.Company.Owner.tournteam
                            DestFac := Town.WorldLocator.FacilityAt( x, y );
                            if (DestFac <> nil) and (not DestFac.Deleted) and (((tournton = true) and (orggate.Block.Facility.Company.Owner.tournteam = DestFac.Company.Owner.tournteam))or(tournton = false)) //and (not DestFac.CriticalTrouble)
                              then
                                begin
                                  if Input
        ...
        if Roads <> nil
          then
            begin
              ...
              Roads.DeleteAll;
              if Facility.Town <> nil
                then Facility.Town.RoadHandler.NearestCircuitsToArea( cirRoads, a,b,c,d, Roads );
            end;
      except
        on E : Exception do
  ...
  *)
  ...
  constructor TTown.Create( aName : string; aXPos, aYPos : integer; aCluster : TCluster; aModelFactory : IModelFactory; aMailServer : IMailServer );
    ...
    begin
      ...
      try
       ...
       logs.Log('New Mayor Mail FAIL',fMayor.Name);
      end;
  ...
  procedure TTown.LoadFromBackup( Reader : IBackupReader );
      ...
      begin
        ...
        for i := 0 to pred(count) do
          begin
            ...
            if Tax <> nil //and (strtoint(tax.MetaTax.Id)<530))
    ...
    begin
      inherited;
     // logs.Log('tttest','load town');
      fid   := Reader.ReadInteger( 'TownId', 0 );
      fName := Reader.ReadString( 'Name', '' );
      xPos  := Reader.ReadInteger( 'xPos', 0 );
      yPos  := Reader.ReadInteger( 'yPos', 0 );
      {if TheGlobalConfigHandler.GetConfigParm('tournteam', '0')='1' then tournp  := Reader.ReadInteger( 'ftournp', 0 );}
  ...
  procedure TTown.StoreToBackup( Writer : IBackupWriter );
    ...
    begin
      inherited;
      //logs.Log('tttest','store town');
      Writer.WriteInteger( 'TownId', fid );
      Writer.WriteString( 'Name', fName );
      Writer.WriteInteger( 'xPos', xPos );
      Writer.WriteInteger( 'yPos', yPos );
      {if TheGlobalConfigHandler.GetConfigParm('tournteam', '0')='1' then Writer.WriteInteger( 'ftournp', ftournp );}
...

{
  ...
  constructor TTycoon.Create( anId : TTycoonId );
    begin
      ...
      fTournamentOn    := StrToInt(TheGlobalConfigHandler.GetConfigParm('TornamentLength', '0')) = 1;
      ftton            := StrToInt(TheGlobalConfigHandler.GetConfigParm('tournteam', '0')) = 1;
      ...
      if TheGlobalConfigHandler.GetConfigParm('tournteam',    '0') = '1' then
        ftournteam := Random(strtoint(TheGlobalConfigHandler.GetConfigParm('towns','1'))) + 1
       else
        ftournteam := 0;
      InitRankings;
  ...
  procedure TTycoon.EndOfPeriod( PeriodType : TPeriodType; PeriodCount : integer );
    ...
    procedure PayExtraTaxes;
      ...
      var
        ...
        TotTax : TMoney;
      begin
        TotTax := 0;      // if PercPerValue<1 then            if minperc < 1 then
        PercPerValue:= strtoint(TheGlobalConfigHandler.GetConfigParm('ifeltaxperc', '10000000'));
        minperc:=  strtoint(TheGlobalConfigHandler.GetConfigParm('ifelminperc', '50'));
        try
                          ...
                          if MT <> nil
                            then
                              begin
                                ...
                                if MainD <> nil
                                  then MainD.GenMoney( tax/10, TA.MetaAccount.AccId );
                              end;
                        end;
                  end;
            end;
  ...
  procedure TTycoon.LoadFromBackup( Reader : IBackupReader );
    ...
    begin
      ...
      {if fEvents = nil
        ...
        logs.Log('loadtycoon',fname + ' events');
      fPolicies := TLockableCollection.Create( 0, rkBelonguer );
      ...
      if not FindResearchItemsInCurriculum // >> CountItemsInCurriculum( currKind_Research ) = 0
       ...
       logs.Log('loadtycoon',fname + ' research');
      // Tutorial
      ...
      {if fTutorial = nil // >> REM
        then InstantiateTutorial;}
      logs.Log('loadtycoon',fname + ' readcookie');
      Reader.ReadObject( 'Cookies', fCookies, nil );
      logs.Log('loadtycoon',fname + ' cookie read');
      if fCookies = nil
        then fCookies := TStringList.Create;
      logs.Log('loadtycoon',fname + ' cookie nil');
      str := Reader.ReadString( 'TycoonLevel', '' );
      logs.Log('loadtycoon',fname + ' level ' + str);
      if TheGlobalConfigHandler.GetConfigParm('LifeAfterLegend', '0') = '1'
      then
        begin
          LevelIn := False;
          if str='' then levelin := true;
      ...
      logs.Log('loadtycoon',fname + ' level');
      ...
      //logs.Log('tttest','votes tycoon');
      Reader.ReadObject('InTowns', fInTowns, nil);
      if fInTowns = nil
        then fInTowns := TCollection.Create(0, rkUse);
     // logs.Log('tttest','intown tycoon');
      Reader.ReadObject( 'Favorites', fFavorites, nil );
      if fFavorites = nil
        then fFavorites := TFavorites.Create;
      //logs.Log('tttest','tycoon loaded');
      // Write reserved (1 taken 2/13/2002)
      for i := 3 to 10 do
        Reader.ReadInteger('Rsvd' + IntToStr(i), 0);
      //logs.Log('tttest','tournteam tycoon');
      // read tourn team
      if TheGlobalConfigHandler.GetConfigParm('tournteam', '0')='1' then
        begin
        ftournteam := strtoint(fCookies.Values['tournteam']);
        ttname := fcookies.Values['ttname'];
        ftton  := true;
        end
      else
        begin
  ...
  procedure TTycoon.StoreToBackup( Writer : IBackupWriter );
    ...
    begin
      try
        ...
        fCookies.Values['tournteam'] := IntToStr(ftournteam);
        fcookies.Values['ttname'] := ttname;
        if fIsDemo
        ...
        Writer.WriteLooseObject( 'Favorites', fFavorites );
         //logs.Log('tttest',' tycoon stored');
        // Save reserved (1 taken)
        for i := 3 to 10 do
          Writer.WriteInteger('Rsvd' + IntToStr(i), 0);
        // write tournteam
       { if TheGlobalConfigHandler.GetConfigParm('tournteam', '0')='1' then Writer.WriteInteger( 'tournteam', tournteam ); }
        //Writer.WriteBoolean( 'ftton',  tton);
      except
  ...
  function TTycoon.CanAdvanceLevel : boolean;
    var
      demo : boolean;
    begin
      Isdemo := false; // no one has demo account
      demo := IsDemo;
      if demo
        then
          begin
  ...
  procedure InitVisualClasses;
    var
      Stream : TStream;
    begin                                       //GetVisualRootPath + 
      Stream := TFileStream.Create( 'classes.bin', fmOpenRead );
      try
