...
interface
  ...
  uses
    Taxes, Kernel, Accounts, CacheAgent, BackupInterfaces, Languages,world;
...
implementation
  ...
  constructor TMetaTaxToAccount.Create( anId : string; aAccountId : TAccountId; aTaxClass : CTax );
    ...
    begin
      ...
      fPercent   := StrToInt(TheGlobalConfigHandler.GetConfigParm('towntax', '10'))/100; //0.1;
  ...
  function TTaxToAccount.GetSubsidized : boolean;
    begin
      if ((world.mintowntax>0) and (world.mintowntax>fpercent)) then fpercent:= world.mintowntax;
      result := fPercent < 0;
    end;
  ...
  procedure TTaxToAccount.SetSubsidized( value : boolean );
    begin
      if value
        then
          begin
          if world.mintowntax>0 then fPercent:= world.mintowntax else fPercent := -1;
          end
        else fPercent := StrToInt(TheGlobalConfigHandler.GetConfigParm('towntax', '10'))/100;
    end;
