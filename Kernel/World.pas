...
interface
  ...
  const
    ...
    MaxDaysToPurge      = 90; //30 for zyrane
  ...
  type
    ...
    TWorld =
      class( TPersistent, ITimer, IMapRefresh, IWorldLocator, IRoadHandler, IModelFactory, ILandInfo, IMatrix )
        ...
        private
          procedure NearestCircuitsToArea( CircuitId : TCircuitId; a,b,c,d:integer; var Circuits : TCollection );
          function  AreasAreConnected( CircuitId : TCircuitId; a,b,c,d, e,f,g,h:integer ) : boolean;
        ...
        public
          ...
          property Tournamentteams  : boolean read fTournamentteams;
...
implementation
 ...
 // var tidSystemPassword : string;
            ...
            while fDeadMeat.Count > 0 do
              begin
                ...
                try
                  ...
                  Logs.Log( 'dm', 'DEAD MEAT: wiping successfull!' );
                except
                  Logs.Log( 'dm', 'DEAD MEAT: Error wiping!' );
                end;
                ...
                try
                  Logs.Log( 'dm', 'DEAD MEAT: Destroying Facility...' );
                  //fDeadMeat.AtDelete( 0 ); // >>> !!!!!!!!!!!
                  fDeadMeat.AtExtract( 0 );
                  Logs.Log( 'dm', 'DEAD MEAT: Success Destroying!' );
                except
                  Logs.Log( 'dm', 'DEAD MEAT: Error!' );
                end;
              end;
          finally
            fDeadMeat.Unlock;
                  ...
                  then
                    begin
                      ...
                      UpdateContestData;
                        ...
                        Logs.Log('tour', TimeToStr(Now) + 'tournament error');
                      end;
                ...
                then
                  begin
                    ...
                    if days > MaxDaysToPurge
                      then
                        begin
                          Logs.Log( 'purge', TimeToStr(Now) + Format(' Purging user %s expire by %d days.', [Tycoon.Name, days]));
  ...
  const
    tbContestPoints : array[0..7] of RankInfo =
    (
      (id: 'NTA'; val1: 20; val2: 18; val3: 16; val4: 14; val5: 12; val6: 10; val7: 8; val8: 6; val9: 4; val10: 2), // NTA
      (id: '4';   val1: 20; val2: 18; val3: 16; val4: 14; val5: 12; val6: 10; val7: 8; val8: 6; val9: 4; val10: 2), // Industry
      (id: '3';   val1: 20; val2: 18; val3: 16; val4: 14; val5: 12; val6: 10; val7: 8; val8: 6; val9: 4; val10: 2), // Commerce
      (id: '7';   val1: 10; val2: 8; val3: 7; val4: 6; val5: 5; val6: 4; val7: 3; val8: 2; val9: 1; val10: 1), // Firms
      (id: '6';   val1: 10; val2: 8; val3: 7; val4: 6; val5: 5; val6: 4; val7: 3; val8: 2; val9: 1; val10: 1), // Offices
      (id: '2';   val1: 10; val2: 8; val3: 7; val4: 6; val5: 5; val6: 4; val7: 3; val8: 2; val9: 1; val10: 1), // Offices
      (id: 'Prestige';   val1: 5; val2: 4; val3: 4; val4: 3; val5: 3; val6: 3; val7: 2; val8: 2; val9: 2; val10: 1), // Prestige
      (id: 'Wealth';   val1: 5; val2: 4; val3: 4; val4: 3; val5: 3; val6: 3; val7: 2; val8: 2; val9: 2; val10: 1) // Wealth
    );

  procedure TWorld.UpdateContestData;

    function AddPts(Tycoon  : TTycoon; field : string; count : integer):integer;
      var
        aux : string;
      begin
        ...
        Tycoon.Cookie[field] := aux;       // kernel.ttown(towns[-1]).tournp    kernel.ttown(towns[-1]).tournp
        //if tournamentteams then  townteampts[tycoon.tournteam]:=townteampts[tycoon.tournteam]  + strtoint(aux);
    ...
    var
      ...
      loanPts : int64;
      p       : integer;
      ttcache : TObjectCache;
      ttrank  : array[0..100] of integer;
      townteampts : array[0..100] of integer;
    begin
      // Award point based on the ranking..
      if tournamentteams then
        begin
          try
            for i:=0 to (towns.Count + 2) do
              begin
              townteampts[i] :=0;
              //ttown(Towns[i]).tournp :=0;
              end;
          except
          logs.Log('tourteam','error in team points to initial 0');
          end;
        end;
      logs.Log('tourteam','start normal points calculations');
                  ...
                  then
                    try
                      ...
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:= AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val1);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:= AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val1);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0 ;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:= AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val2);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:= AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val2);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val3);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val3);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val4);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val4);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val5);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val5);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val6);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val6);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                 ...
                 if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val7);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val7);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                ...
                if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val8);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val8);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                 ...
                 if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val9);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val9);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
                 ...
                 if count > p
                  then
                    try
                      x:=0;
                      if Ranking.IsRankeable(Ranking.Objects[p])
                        then x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val10);
                      if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                      inc(p);
                      while (p < count) and Ranking.IsRankeable(Ranking.Objects[p]) and (Ranking.CompareObjects(Ranking.Objects[p-1], Ranking.Objects[p], self) = 0) do
                        begin
                          x:=AddPts(TTycoon(Ranking.Objects[p]), 'rkPts', tbContestPoints[i].val10);
                          if tournamentteams then  townteampts[TTycoon(Ranking.Objects[p]).tournteam]:=townteampts[TTycoon(Ranking.Objects[p]).tournteam]  + x;
                          inc(p);
                        end;
                    except
              ...
              then
                begin
                  if (hiLevel > 0) and (Tycoon.Level.Tier = hiLevel)
                    then
                      begin
                        Tycoon.Cookie['lvPts'] := '40'; // const     kernel.ttown(towns[-1]).tournp  kernel.ttown(towns[-1]).tournp
                        if tournamentteams then  townteampts[tycoon.tournteam]:= townteampts[tycoon.tournteam] + 40;
                      end
                  ...
                  if loanPts > 0
                    then
                      begin
                        Tycoon.Cookie['bnkPts'] := IntToStr(-loanPts);                   //  kernel.ttown(towns[-1]).tournp   kernel.ttown(towns[-1]).tournp
                        if tournamentteams then  townteampts[tycoon.tournteam]:=  townteampts[tycoon.tournteam]-loanpts;
                      end
     ...
     // add town team info here
     logs.Log('tourteam','end normal points calclations');
     if tournamentteams then
      try
        begin
         //work out town rankings
         logs.Log('tourteam','');
         logs.Log('tourteam','start next tourn team calculations:');
         for i:=0 to (towns.Count + 2) do
          begin
            ttrank[i] := i;
            k:=i;                        // kernel.ttown(Towns[i]).Name    kernel.ttown(Towns[i]).tournp
            logs.Log('tourteam', 'team' + inttostr(i+1) + ' - ' + inttostr(townteampts[i+1]));
            for j:=0 to pred(i) do
              begin          // kernel.ttown(Towns[]).tournp    kernel.ttown(Towns[]).tournp
                 if townteampts[k] > townteampts[j] then
                  begin
                    ttrank[k]:= j;
                    ttrank[j]:= k;
                    k:= j;
         ...
         //cache town rankngs
         {
         tidCachePath_Rankings + Ranking.Id + '.five\'
          RankingName4=NTA
          RankingName3=NTA
          RankingName2=ANMI
          RankingName1=NTA
         ...
         logs.Log('tourteam','start caching');
         ttcache:= TObjectCache.Create;
         ttcache.Update := true;
         ttcache.Path:= tidCachePath_Rankings + 'teamtourn.five\';
         ttcache.WriteString('Path',tidCachePath_Rankings + 'teamtourn.five\');
         ttcache.WriteString('Count',inttostr(towns.Count));
         ttcache.WriteString('ID','teamtourn');
         ttcache.WriteString('RankingName0','Team Tournament');
         ttcache.WriteString('RankingName1','Team Tournament');
         ttcache.WriteString('RankingName2','Team Tournament');
         ttcache.WriteString('RankingName3','Team Tournament');
         ttcache.WriteString('RankingName4','Team Tournament');
         for i:=1 to (towns.count) do
          begin       //    kernel.ttown(Towns[ttrank[i]]).Name     kernel.ttown(Towns[]).tournp
             ttCache.WriteString( 'Name' + IntToStr(i), 'Team ' + inttostr(ttrank[i]+1) );
             ttCache.WriteString( 'Value' + IntToStr(i), inttostr(townteampts[ttrank[i]]));
          end;
         ttcache.Destroy;
         logs.Log('tourteam','end caching');
        end;
      except
      end;
    end;
      ...
      try
        ...
        while ((number = 0) or (i < number)) and (i < ZoningMessages.Count) do
          begin
            ...
            inc(i);
            Logs.Log( 'zonning', TimeToStr(Now) + Format('Sent Zoning Message to : %s' , [Msg.fZoned]));
      ...
      try
        ...
        try
          try
            ...
            if tournamentteams then
              begin
               if company.Owner.tournteam = town.Id then tt:=1 else tt:=0;
              end
            else
              tt:=1;
  ...
  function TWorld.RDOConnectFacilities( Owner : TTycoon; Facility1, Facility2 : TFacility ) : OleVariant;
    begin
      ...
      try
        try
          if ((tournamentteams = true) and (facility1.Company.Owner.tournteam = facility2.Company.Owner.tournteam)) or (tournamentteams = false) then
  ...
  function TWorld.RDONewTycoon( name, password : widestring ) : OleVariant;
    begin
      Logs.Log( 'tycoon', TimeToStr(Now) + ' New Tycoon: ' + Name + ', ' + password );
  ...
  function TWorld.RDODelTycoon( name, password : widestring ) : OleVariant;
    var
      Tycoon : TTycoon;
    begin
      Logs.Log( 'tycoon', TimeToStr(Now) + ' Del Tycoon: ' + Name + ', ' + password );
      try
        Tycoon := TycoonByName[name];
        if Tycoon <> nil
          then
  ...
  function TWorld.RDOResetTycoon( name : widestring ) : OleVariant;
    begin
      Logs.Log( 'tycoon', TimeToStr(Now) + ' Reset Tycoon: ' + Name + ' (Failed Attempt!!)');
    end;
  ...
  function TWorld.RDOResetTycoonEx( name, password : widestring ) : OleVariant;
    var
      Tycoon : TTycoon;
    begin
      Logs.Log( 'tycoon', TimeToStr(Now) + ' Reset Tycoon: ' + Name );
      try
        Tycoon := TycoonByName[name];
        if (Tycoon <> nil) and (UpperCase(Tycoon.Password) = UpperCase(password))
          then
            ...
            begin
              Logs.Log( 'tycoon',  'ERROR in Reset Tycoon (Trying to hack)');
              result := ERROR_UnknownTycoon;
            end;
        Logs.Log( 'tycoon',  'OK!');
      except
        result := ERROR_Unknown;
        Logs.Log( 'tycoon',  'ERROR in Reset Tycoon');
      end;
    end;
                  ...
                  begin
                    ...
                    if Cluster <> nil
                      then
                        begin
                          Tycoon := TycoonByName[username];
                          if tycoon.Level = nil then tycoon.Level := TTycoonLevel(TheClassStorage.ClassById[tidClassFamily_TycoonLevels, tidTycoonLevel_Apprentice]);
    ...
    begin
      ...
      try
        try
            ...
            then
              begin
                ...
                if Tycoon <> nil
                  then
                    begin
                      Logs.Log( 'tycoon', TimeToStr(Now) + ' Tycoon logged on: ' + Tycoon.Name + '(' + IntToStr(TycoonId) + ')' );
                      try
          ...
          Logs.Log( 'tycoon', TimeToStr(Now) + ' Error Awaking Tycoon');
        end;
      finally
        fWorldLock.Leave;
      end;
  ...
  procedure TWorld.RDOSleepTycoon( TycoonId : integer );
    ...
    begin
      //Logs.LogMemReport(tidLog_Survival);
      Logs.Log( 'tycoon', TimeToStr(Now) + ' SleepTycoon: ' + IntToStr(TycoonId) );
      fWorldLock.Enter;
      try
        try
          Tycoon := TycoonById[TycoonId];
          if Tycoon <> nil
            then
              begin
                Logs.Log( 'tycoon', TimeToStr(Now) + ' Tycoon logged off: ' + Tycoon.Name + '(' + IntToStr(TycoonId) + ')' );
                Tycoon.Sleep;
                Logs.Log( 'tycoon',  'OK!');
              end;
          AwakenTycoon := GetAwakenTycoon( TycoonId );
        ...
        Logs.Log( 'tycoon',  'OK!');
          ...
          on E : Exception do
            Logs.Log( 'tycoon', TimeToStr(Now) + ' Error SleepTycoon ' + E.Message );
        end;
      finally
        fWorldLock.Leave;
      end;
  ...
  procedure TWorld.NearestCircuitsToArea( CircuitId : TCircuitId; a,b,c,d:integer; var Circuits : TCollection );
    var
      Circuit   : TCircuitMap;
      ErrorCode : TCircuitErrorCode;
      Area:trect;
  ...
  function TWorld.AreasAreConnected( CircuitId : TCircuitId; a,b,c,d, e,f,g,h:integer ) : boolean;
    var
      Circuit   : TCircuitMap;
      ErrorCode : TCircuitErrorCode;
      useless,Area1,Area2   : TRect;
    begin
      try
        Circuit := CircuitById[CircuitId];
        Area1:=rect(a,b,c,d);
      ...
      try
        ...
        try
          if (fTournamentLength = 0) and (Tycoon.Companies.Count = 0) and not Tycoon.IsRole and not VarIsEmpty( DirProxy )
            then
              begin
                canJoinWorld := true;//DirProxy.RDOCanJoinNewWorld(Tycoon.Name);
  ...
  function TWorld.NewTycoon( name, password : string ) : TTycoon;
    ...
    begin
      ...
      if TheGlobalConfigHandler.GetConfigParm('tournteam',    '0') = '1' then
        result.ttname := kernel.TTown(towns[result.tournteam-1]).Name
      else
        result.ttname := 'none';
  ...
  procedure TWorld.LoadFromBackup( Reader : IBackupReader );
    ...
    begin
      ...
      if TheGlobalConfigHandler.GetConfigParm('RelayEconomy', '0') = '1'
        ...
        if TheGlobalConfigHandler.GetConfigParm('removeRelay', '0') = '1' then fEcconomyRelay := nil;
      SynchronizeClock;  
    end;
  ...
  procedure TWorld.Loaded( Notify : TBackupReaderNotify );
    ...
    begin
      ...
      fTournamentteams  := TheGlobalConfigHandler.GetConfigParm('tournteam',    '0') = '1';
      kernel.tournton := Tournamentteams;
