...
implementation
            ...
            begin
              ...
              if not LowCost //and (Facility.ToBeDemolished = 0)
                then
                  begin
                  //invention check
                  if (finvCrimeRes>1) or (finvCrimeRes<-1) then finvCrimeRes:=finvCrimeRes/10;
                  if (finvPollRes>1) or (finvPollRes<-1) then finvPollRes:=finvPollRes/10;
                  if (finvBeauty>1) or (finvBeauty<-1) then finvBeauty:=finvBeauty/10;
                  if (finvPrivacy>1) or (finvPrivacy<-1) then finvPrivacy:=finvPrivacy/10;
                  if (finvMaint>1) or (finvMaint<-1) then finvMaint:=finvMaint/10;
                    ...
                    //if Admitance >160 then Admitance :=160;
                  end
                else Admitance := 0;
  ...
  procedure TPopulatedBlock.RecalculateInventionsEffect;
    ...
    logs.Log('resinv','start');
      ...
      logs.Log('resinv','end - ' + floattostr(finvCrimeRes) + ' - ' + floattostr(finvPollRes) + ' - ' + floattostr(finvBeauty) + ' - ' + floattostr(finvPrivacy) + ' - ' + floattostr(finvMaint) + ' - ');
    end;
