...
implementation
  ...
  procedure TTownTaxInfo.LoadFromBackup( Reader : IBackupReader );
    begin
      ...
      if ((world.mintowntax>0) and (world.mintowntax>ftaxvalue)) then  ftaxvalue := world.mintowntax;
      ...
      begin
        ...
        for i := 0 to pred(Towns.Count) do
          begin
            ...
            TX.TaxValue := StrToInt(TheGlobalConfigHandler.GetConfigParm('towntax', '10'))/100;// 0.1;
  ...
  procedure TPresidentialHall.RDOVote(voterTycoon, choiceTycoon : widestring);
    var
      Voter, Votee : TTycoon;
    begin
      Logs.Log( 'vote', DateTimeToStr(Now) + Format(' Voting: %s by %s', [voterTycoon, choiceTycoon]));
      try
        Voter := fWorld.TycoonByName[voterTycoon];
        Votee := fWorld.TycoonByName[choiceTycoon];
        if (Voter <> nil) and (Voter.AllCompaniesCount > 0) and not Voter.IsRole and (Votee <> nil) and Voter.CheckOpAuthenticity and Voter.PayTaxes and (voter.Level.Tier > 1)
          then
            begin
        ...
        Logs.Log( 'vote', DateTimeToStr(Now) + ' error in RDOVote..');
      end;
    end;
      ...
      begin
        ...
        with TPoliticalWorld( Obj ) do
          begin
            ...
            result.WriteBoolean('Tournamentteams', Tournamentteams);
